#!/usr/bin/env bash
# =============================================================================
# deploy.sh — SLIC Voice Agent (Nimali) Deployment Script
# =============================================================================
# Usage:
...
cmd_setup() {
  check_config
  info "Setting up $SERVER_IP for SLIC Agent deployment..."
...
UFW_SETUP
  ...
  run_remote "bash -s" <<NGINX_TEMP
    cat > /etc/nginx/sites-available/slic-agent <<'CONF'
server {
    listen 80;
    server_name ${DOMAIN};
    location / {
        return 200 'SLIC Agent setup in progress';
        add_header Content-Type text/plain;
    }
}
CONF
    ln -sf /etc/nginx/sites-available/slic-agent /etc/nginx/sites-enabled/
...
NGINX_TEMP
  ...
  run_remote "sed 's/YOUR_DOMAIN/${DOMAIN}/g' ${REMOTE_DIR}/nginx.conf > /etc/nginx/sites-available/slic-agent && nginx -t && systemctl reload nginx"
...
cmd_logs() {
  check_config
  run_remote "cd ${REMOTE_DIR} && docker compose logs -f slic-agent"
}
