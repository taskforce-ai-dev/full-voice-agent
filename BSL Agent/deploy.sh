#!/usr/bin/env bash
# =============================================================================
# deploy.sh — BSL Voice Agent (Bank of Sri Lanka) Deployment Script
# =============================================================================
# Usage:
...
DOMAIN="bsl.taskforceai.tech"
EMAIL="info@taskforceai.tech"
# ======================================================

SERVER_USER="root"
REMOTE_DIR="/opt/bsl-agent"
...
cmd_setup() {
  check_config
  info "Setting up $SERVER_IP for BSL Agent deployment..."
...
UFW_SETUP
  ...
  # Temporary nginx config for certbot (HTTP-01 challenge needs port 80 reachable)
  run_remote "bash -s" <<NGINX_TEMP
    cat > /etc/nginx/sites-available/${NGINX_SITE} <<'CONF'
server {
    listen 80;
    server_name ${DOMAIN};
    location / {
        return 200 'BSL Agent setup in progress';
...
NGINX_TEMP
  ...
  # Install full nginx config (overwrites the temporary one)
  run_remote "cp ${REMOTE_DIR}/nginx.conf /etc/nginx/sites-available/${NGINX_SITE} && nginx -t && systemctl reload nginx"
...
cmd_deploy() {
  ...
  info "Rebuilding and recreating container (--force-recreate so .env is re-read)..."
  run_remote "cd ${REMOTE_DIR} && docker compose build && docker compose up -d --force-recreate bsl-agent"
...
cmd_logs() {
  check_config
  run_remote "cd ${REMOTE_DIR} && docker compose logs -f bsl-agent"
}
