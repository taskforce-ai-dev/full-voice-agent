#!/usr/bin/env bash
# =============================================================================
# deploy.sh — Hatton Hills Voice Agent Deployment Script
# =============================================================================
# Usage:
...
DOMAIN="worldofrefrigerators.taskforceai.tech"
EMAIL="info@taskforceai.tech"
# ======================================================

SERVER_USER="root"
REMOTE_DIR="/opt/worldofrefrigerators"
...
cmd_setup() {
  check_config
  info "Setting up $SERVER_IP for World Of Refrigerators deployment..."
...
UFW_SETUP
  ...
  run_remote "bash -s" <<NGINX_TEMP
    cat > /etc/nginx/sites-available/worldofrefrigerators <<'CONF'
server {
    listen 80;
    server_name ${DOMAIN};
    location / {
        return 200 'World Of Refrigerators setup in progress';
        add_header Content-Type text/plain;
    }
}
CONF
    ln -sf /etc/nginx/sites-available/worldofrefrigerators /etc/nginx/sites-enabled/
...
NGINX_TEMP
  ...
  run_remote "sed 's/YOUR_DOMAIN/${DOMAIN}/g' ${REMOTE_DIR}/nginx.conf > /etc/nginx/sites-available/worldofrefrigerators && nginx -t && systemctl reload nginx"
...
cmd_logs() {
  check_config
  run_remote "cd ${REMOTE_DIR} && docker compose logs -f worldofrefrigerators"
}
