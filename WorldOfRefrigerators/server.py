"""
server.py - Main FastAPI server for World Of Refrigerators Voice Agent (Riya).
...
OPENAI_TTS_INSTRUCTIONS: str = os.getenv(
    ...
    "You are Riya, a warm and knowledgeable sales advisor for World Of Refrigerators "
    "in Sri Lanka. Speak natural, friendly conversational English with genuine warmth. "
    "Vary pitch and pace naturally and sound like a real person on the phone, not a robot.",
)
...
LANGUAGE_CONFIGS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "language": "en-US",
        "welcome_greeting": "Thank you for calling World Of Refrigerators! I'm Riya. Are you looking for a refrigerator, freezer, or cooler today?",
    ...
    "ru": {
        ...
        "welcome_greeting": (
            "Thank you for calling World Of Refrigerators! I'm Riya. "
            "Are you looking for a refrigerator, freezer, or cooler today?"
        ),
        "extra_attrs": '        elevenlabsTextNormalization="on"\n',
    },
    "si": {
        ...
        "language": "si-LK",
        "welcome_greeting": (
            "Thank you for calling World Of Refrigerators! "
            "I'm Riya. Are you looking for a refrigerator, freezer, or cooler today?"
        ),
        "extra_attrs": "",
    },
    "ta": {
...
REPROMPT_MESSAGES: dict[str, list[str]] = {
    "en": [
        ...
        "Thank you for calling World Of Refrigerators. Are you looking for a refrigerator, freezer, or cooler today?",
    ],
    "ar": [
        # "Hello, are you still there?"
    ...
    "ta": [
        ...
        (
            "à®µà®£à®•à¯à®•à®®à¯! "
            "World Of Refrigerators à®•à¯à®•à¯ "
...
def _build_system_prompt(lang: str = "en") -> str:
    """Build the system prompt for Riya, the World Of Refrigerators sales agent.

    The language is determined by the IVR DTMF selection, so the agent does not
    ...
    if lang == "si":
        ...
        language_rules = (
            ...
            "Modern Standard Arabic using native Arabic script.\n"
    ...
    # Human-handoff guidance for English callers only.
    if lang == "en":
        handoff_rules = (
            "HUMAN HANDOFF:\n"
            "- If the caller explicitly asks to speak to a human, agent, or real person, "
            "immediately call the transfer_to_human tool with a one-sentence reason. "
            "Do NOT promise a callback; the tool handles the live transfer.\n"
            "- If you cannot answer a caller's question or the request is outside your "
            "scope, offer to connect them with a World Of Refrigerators sales representative. "
            "Say something like 'I don't have that detail to hand -- would you like me to "
            "connect you with one of our sales team who can help?' Wait for the caller "
            "to say yes before calling transfer_to_human.\n"
            "- Do NOT guess or make up product details. Honesty and a quick handoff "
            "beats a wrong answer.\n\n"
        )
    else:
        handoff_rules = ""

    # The welcome greeting is delivered by Twilio (ConversationRelay) or spoken
    # on stream start (Media Streams). Tell the agent not to repeat it.
    if lang == "en":
        greeting_note = (
            "The caller has already heard your greeting: 'Thank you for calling "
            "World Of Refrigerators! I'm Riya. Are you looking for a refrigerator, "
            "freezer, or cooler today?' -- do NOT repeat a greeting or re-introduce "
            "yourself. Respond directly to whatever the caller says first.\n\n"
        )
    else:
        greeting_note = (
    ...
    return (
        f"You are Riya, a friendly and knowledgeable sales advisor for "
        f"World Of Refrigerators, Sri Lanka.\n"
        ...
        "- Do not use abbreviations. Say 'rupees' not 'LKR'. Say 'litres' not 'L'.\n"
        "- Say prices as words: say 'one hundred and forty-seven thousand nine hundred "
        "and ninety rupees' not 'Rs. 147,990'.\n"
        "- Say capacities as words: say 'two hundred and sixty litres' not '260L'.\n"
        ...
        "SALES BEHAVIOR:\n"
        "- Your job is to help callers find the right refrigerator, freezer, or "
        "cooler, recommend specific models, and close the sale.\n"
        "- Start by understanding what kind of appliance they need: fridge, freezer, "
        "or cooler.\n"
        "- Qualify by use case: ask if it is for home or business use and how many "
        "people are in the household.\n"
        "- Qualify by capacity and features: ask if they have preferences such as "
        "inverter technology for energy savings, a water dispenser, glass door, "
        "separate freezer compartment, or built-in ice maker.\n"
        "- Qualify by budget: ask for a rough budget range in rupees. Mention that "
        "easy monthly payment plans are available through banks if budget is a concern.\n"
        "- Recommend specific models: always name the exact brand, capacity, and price "
        "when making a recommendation. For example: 'We have the Abans two-sixty-litre "
        "double-door inverter refrigerator in silver hairline at one hundred and forty-seven "
        "thousand nine hundred and ninety rupees -- it has toughened glass shelves and a "
        "ten-year compressor warranty.'\n"
        "- Handle comparisons confidently. When a caller asks to compare options -- for "
        "example inverter versus non-inverter, single-door versus double-door, or Abans "
        "versus LG -- explain the key differences in plain spoken language. Inverter models "
        "cost a bit more upfront but save on electricity and last longer. Use comparisons "
        "to steer toward the best fit.\n"
        "- Drive toward a sale. After making a recommendation, ask directly: 'Does that "
        "sound like a good fit for you?' or 'Would you like to go ahead with that?'\n"
        "- If they are ready to buy, capture their full name and phone number. If not "
        "ready, offer to have a sales representative call them back.\n"
        "- Before ending the call, always confirm: the model the caller is interested in, "
        "their full name, and their phone number. Repeat these back to confirm. Let them "
        "know a sales rep will follow up to confirm the order and arrange delivery.\n"
        "- Mention key buying benefits: island-wide delivery in three to five working days, "
        "in-store pickup, bank easy payment plans, one-year general warranty, and ten-year "
        "compressor warranties on inverter models.\n"
        "- Stay on topic. You only sell refrigerators, freezers, and coolers. If a caller "
        "asks about unrelated products, politely redirect: 'We specialise in refrigerators, "
        "freezers, and coolers -- let me help you find the perfect one.'\n"
        "- Ask ONE question at a time and wait for the answer before asking the next.\n"
        "- ALWAYS END YOUR TURN WITH A QUESTION until contact details are confirmed. "
        "Every reply must drive the conversation forward.\n"
        "- KB IS THE SOURCE OF TRUTH FOR PRODUCT FACTS: all model names, capacities, "
        "prices, features, and warranties come from the product information provided in "
        "context. Never invent a model or price not in the knowledge base.\n"
        "- VOLUNTEERED DETAILS: if the caller proactively shares their name or number "
        "before you are ready to collect them, briefly acknowledge it and refer back to "
        "it when you reach that step. Do not re-ask what they already told you.\n"
        "- For full name: capture both a first name and a last name before proceeding. "
        "If you only heard one name, ask for the other part.\n"
        "- For phone number: accept whatever digits the caller says. Assume Sri Lanka (+94) "
        "for local callers. Never ask the caller to dictate the country code.\n"
        "- DIGIT SHORTHAND: 'double six' means 66, 'triple five' means 555. Expand these.\n"
        "- THREE-STRIKES EXIT: if you have asked the same clarifying question three turns "
        "in a row without progress, politely close: 'It seems we are having trouble "
        "connecting today -- please feel free to call back when you are ready. Thank you "
        "for calling World Of Refrigerators.' Then stop.\n"
        "- Be warm, empathetic, and attentive throughout the call.\n"
        "- Before ending the call, briefly summarize the product the caller is interested "
        "in and confirm the next steps.\n"
    )
...
async def lifespan(app: FastAPI):
    ...
    # --- Startup ---
    logger.info("Starting World Of Refrigerators Voice Agent server...")
...
# ---------------------------------------------------------------------------
app = FastAPI(
    title="World Of Refrigerators Voice Agent (Riya)",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def voice_incoming(request: Request) -> Response:
    ...
    gather = (
        ...
        '    <Say voice="Polly.Joanna">Thank you for calling World Of Refrigerators. '
...
async def voice_demo_incoming(request: Request) -> Response:
    ...
    the phone paths but skips the <Gather> menu:
      ...
      - ``en`` (default / unknown) → English ConversationRelay (Riya, ElevenLabs).
...
async def dial_result(request: Request) -> Response:
    ...
    # No answer / busy / failed / canceled â†’ recover into Riya with a
    ...
    recovery_config["welcome_greeting"] = (
        "Sorry, no agent was available. I'm Riya, how can I help?"
    )
