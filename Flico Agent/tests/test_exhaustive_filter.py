...
_BEDS = [None] + sorted(set(BEDROOMS) | {6})
...
def db(tmp_path_factory):
    ...
    # truth_property honours the real portfolio's nulls: 24 POR rents,
    # 7 bed-less commercial rows, 2 bath-less rows, 9 null furnishings.
    d.insert_properties_batch([(truth_property(r), zero) for r in TRUTH])
    return d
...
@pytest.mark.parametrize("min_beds", sorted(set(BEDROOMS) | {6}))
