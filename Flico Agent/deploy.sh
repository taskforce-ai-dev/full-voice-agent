#!/usr/bin/env bash
# =============================================================================
# deploy.sh — Flico Voice Agent Deployment Script
# =============================================================================
# Usage:
...
cmd_setup() {
  check_config
  info "Setting up $SERVER_IP for Flico deployment..."
...
UFW_SETUP
  ...
  run_remote "bash -s" <<NGINX_TEMP
    cat > /etc/nginx/sites-available/flico <<'CONF'
server {
    listen 80;
    server_name ${DOMAIN};
    location / {
        return 200 'Flico setup in progress';
        add_header Content-Type text/plain;
    }
}
CONF
    ln -sf /etc/nginx/sites-available/flico /etc/nginx/sites-enabled/
...
NGINX_TEMP
  ...
  run_remote "sed 's/YOUR_DOMAIN/${DOMAIN}/g' ${REMOTE_DIR}/nginx.conf > /etc/nginx/sites-available/flico && nginx -t && systemctl reload nginx"
...
cmd_logs() {
  check_config
  run_remote "cd ${REMOTE_DIR} && docker compose logs -f flico"
}
