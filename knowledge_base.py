"""
knowledge_base.py — Optimized RAG / Knowledge-Base module for the Winrich Hotel Voice Agent.
...
Key optimizations over the baseline implementation:
  * prewarm()            — eagerly loads ChromaDB + embedding model to cut cold-start latency.
  * _cached_embed_query  — LRU-cached query embeddings so repeated questions are instant.
"""
...
def _get_chroma_client():
    """Return (or create) a persistent ChromaDB client — lazy singleton."""
    global _chroma_client
    if _chroma_client is not None:
        return _chroma_client

    if not CHROMADB_AVAILABLE:
        logger.warning("chromadb is not installed — knowledge base disabled.")
        return None
    ...
    try:
        ...
        # PersistentClient doesn't exist in very old versions — fall through
        pass
...
def _get_embedding_model():
    """Return (or create) the SentenceTransformer embedding model — lazy singleton."""
...
def _stable_id(text: str, index: int) -> str:
    ...
    Using a stable ID means re-indexing the same content is idempotent —
...
def _read_file(path: str) -> Optional[str]:
    ...
    Supports:
      * .txt / .md  — read as UTF-8 text (with latin-1 fallback).
      * .pdf        — extract text via PyPDF2 (if installed).
    ...
    if ext == ".pdf":
        if not PYPDF2_AVAILABLE:
            logger.warning("PyPDF2 not installed — cannot read %s", path)
            return None
        try:
...
def initialize_kb(docs_directory: str = DEFAULT_DOCS_DIRECTORY) -> bool:
    ...
    The operation is **idempotent** — chunks are assigned stable IDs so
    ...
    if client is None:
        logger.error("ChromaDB client unavailable — cannot initialise KB.")
        return False
    ...
    logger.info(
        "KB initialisation complete — %d total chunks in collection '%s'.",
        total_chunks,
        COLLECTION_NAME,
    )
    return True
...
def add_document(
    ...
    *metadata* is an optional dict of key-value pairs stored alongside the
    ...
    client = _get_chroma_client()
    if client is None:
        logger.error("ChromaDB client unavailable — cannot add document.")
        return False
    ...
    if not chunks:
        logger.warning("add_document called with empty text — nothing to add.")
        return False
...
# ---------------------------------------------------------------------------
# Module-level initialisation — ensure the ChromaDB client is ready early.
# ---------------------------------------------------------------------------
_get_chroma_client()
