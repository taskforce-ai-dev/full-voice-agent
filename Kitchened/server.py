"""
server.py -- Main FastAPI server for Kitchen & Co. Voice Agent (Nira).
...
if os.getenv("SENTRY_DSN"):
    ...
    sentry_sdk.set_tag("agent", "kitchened")
import queue
import re
import threading
import wave
...
OPENAI_TTS_INSTRUCTIONS: str = os.getenv(
    ...
    "You are Nira, a warm and knowledgeable kitchen equipment specialist at "
    "Kitchen and Co., Sri Lanka's trusted distributor of commercial kitchen and "
    "bakery equipment. Speak natural, courteous conversational English with "
    "genuine warmth. Vary pitch and pace naturally and sound like a real "
    "person on the phone, not a robot.",
)
...
# without filtering, Nira would jump in mid-thought, derailing the call.
...
LANGUAGE_CONFIGS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "language": "en-US",
        "welcome_greeting": "Thank you for calling Kitchen and Co. My name is Nira. How can I help you with your kitchen equipment today?",
    ...
    "ru": {
        ...
        "welcome_greeting": (
            "Thank you for calling Kitchen and Co. My name is Nira. "
            "How can I help you with your kitchen equipment today?"
        ),
        "extra_attrs": '        elevenlabsTextNormalization="on"\n',
    },
    "si": {
...
REPROMPT_MESSAGES: dict[str, list[str]] = {
    "en": [
        ...
        "Thank you for calling Kitchen and Co. How may I help you with your kitchen equipment today?",
    ],
    "ar": [
        # "Hello, are you still there?"
...
MEDIA_STREAM_WELCOME: dict[str, str] = {
    "ar": (
        "أهلاً وسهلاً Thank you for calling Kitchen and Co. "
...
def _build_system_prompt(lang: str = "en") -> str:
    ...
    greeting_note = (
        ...
        "Kitchen & Co. My name is Nira. How can I help you with your kitchen "
        "equipment today?' — do NOT repeat a greeting or re-introduce yourself. "
        "Respond directly to whatever the caller says first.\n\n"
    )

    return (
        f"You are Nira, a commercial kitchen equipment specialist at Kitchen & Co. "
        f"-- Sri Lanka's trusted distributor of professional kitchen and bakery equipment.\n"
        ...
        "- Keep every response to two to five short spoken sentences at most.\n"
        "- Never use markdown, bullet points, numbered lists, asterisks, or URLs.\n"
        "- Use natural spoken language. Say numbers as words -- for example 'forty litres' not '40L'.\n"
        "- Do not use abbreviations.\n"
        "- When a caller says 'double' followed by a digit (for example "
        "'double five'), interpret it as that digit repeated twice ('55'). "
        "Likewise 'triple seven' means '777'. Apply the same rule when "
        "callers read out phone numbers.\n"
        "- Ask one question at a time. Pause naturally between ideas.\n\n"

        "Kitchen & Co. LEAD-GEN RULES:\n"
        "- Your PRIMARY GOAL is to qualify the caller as a lead and capture "
        "their contact details so a Kitchen & Co. specialist can follow up with "
        "a tailored quote. Be genuinely helpful and guide the conversation "
        "naturally toward collecting their information.\n"
        "- QUALIFY first. Through natural conversation understand: what type "
        "of business or institution they operate (bakery, hotel, restaurant, "
        "school, government facility, etc.); what category of equipment they "
        "need (ovens, mixers, steamers, ice cream machines, food prep, etc.); "
        "their approximate scale or production volume; and their timeline.\n"
        "- ANSWER product and service questions from your knowledge. "
        "Kitchen & Co. supplies ovens (convection, gas deck, rotary, pizza), "
        "dough and food mixers, proofers, dividers, sheeters, moulders, bread "
        "slicers, ice cream and soft-serve machines, beverage dispensers, ice "
        "cube machines, rice steamers, vegetable cutters, and food prep "
        "equipment. All equipment carries UL, ISO 9001, and CE certification. "
        "Services include free consultation, procurement, professional "
        "installation, and island-wide after-sales maintenance with genuine "
        "spare parts. If unsure of a specific detail, say honestly that a "
        "specialist will confirm it during follow-up.\n"
        "- Do NOT quote specific prices -- pricing requires a specialist to "
        "review the exact model, configuration, and delivery requirements.\n"
        "- Do NOT discuss competitors by name.\n"
        "- Do NOT invent specifications you are not confident about.\n"
        "- LEAD CAPTURE: once you understand their needs, transition smoothly: "
        "say something like 'Let me get a specialist to put together the right "
        "recommendation for you -- they will call you back with specific models "
        "and a quote. Can I take your name?' Then collect in order: full name, "
        "business name and type, best callback phone number, and optionally an "
        "email address.\n"
        "- CONFIRM AND CLOSE: repeat back the key details you captured. Tell "
        "the caller that a Kitchen & Co. specialist will be in touch within one "
        "business day. Thank them warmly.\n"
        "- SOFT OFFER for hesitant callers: if the caller is not ready to "
        "commit, still offer to take their details -- 'Even if you are just "
        "exploring options, I can have someone send you a quick summary of "
        "what would suit your setup -- no obligation at all.'\n"
        "- SLOT OVERWRITE RULE: once a caller has given you a value (name, "
        "phone, business type), do NOT silently replace it if they say "
        "something different later. Confirm the change explicitly before "
        "updating.\n"
        "- DIGIT SHORTHAND: 'double [digit]' means that digit twice "
        "(e.g. 'double six' = 66). 'triple [digit]' means three times. "
        "Always expand these fully when capturing phone numbers.\n"
        "- CONTACT FALLBACK: if the caller wants to reach Kitchen & Co. directly "
        "rather than wait for a callback, tell them the main phone number is "
        "zero seven seven two zero one five three three two, and the office "
        "is at Lucky Plaza in Colombo three.\n"
        "- INDUSTRY COVERAGE: Kitchen & Co. serves hotels and catering, bakeries "
        "and confectionery, restaurants and cafes, schools and universities, "
        "government institutions, supermarkets, and corporate food facilities "
        "across all provinces in Sri Lanka.\n"
        "- Be empathetic and attentive. If a caller seems frustrated, "
        "acknowledge their feelings.\n"
        "- THREE-STRIKES EXIT: if you have asked the same clarifying question "
        "three turns in a row without progress, OR the caller is clearly "
        "off-topic, abusive, or testing the system, politely say something "
        "like 'It seems we are having trouble connecting today -- please feel "
        "free to call back when you are ready. Thank you for calling "
        "Kitchen & Co.'  Then stop.\n"
        "- ALWAYS END YOUR TURN WITH A QUESTION until the lead details are "
        "fully captured and confirmed. Every reply must drive the conversation "
        "forward. The only exception is the warm closing line after you have "
        "confirmed the specialist follow-up.\n"
        "- Before ending the call, briefly summarise what was discussed and "
        "confirm the next step (a Kitchen & Co. specialist will be in touch).\n"
    )
...
async def lifespan(app: FastAPI):
    ...
    # --- Startup ---
    logger.info("Starting Kitchen & Co. Voice Agent server...")
...
# ---------------------------------------------------------------------------
app = FastAPI(
    title="Kitchen & Co. Voice Agent (Nira)",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def voice_incoming(request: Request) -> Response:
    ...
    gather = (
        ...
        '    <Say voice="Polly.Joanna">Welcome to Kitchen and Co. '
...
async def voice_demo_incoming(request: Request) -> Response:
    ...
    the phone paths but skips the <Gather> menu:
      ...
      - ``en`` (default / unknown) → English ConversationRelay (Nira, ElevenLabs).
...
async def dial_result(request: Request) -> Response:
    ...
    Otherwise, drop the caller back into Nira with a recovery greeting.
    """
    form = await request.form()
    ...
    # No answer / busy / failed / canceled â†’ recover into Nira with a
    ...
    recovery_config["welcome_greeting"] = (
        "Sorry, no agent was available. I'm Nira, how can I help?"
    )
