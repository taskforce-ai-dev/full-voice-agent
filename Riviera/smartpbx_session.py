"""Dialog SmartPBX lifecycle adapter for Riya's existing media pipeline."""
...
class RivieraSmartPBXSession:
    """Own one Dialog call while reusing Riya's media-session behavior."""
    ...
    async def feed_dtmf(self, digit: str) -> bool:
        ...
        if self._selected_language is None:
            ...
            if digit == "2":
                ...
                return True
            if digit == "3":
                await self._activate_language("ta", "digit")
                return True
    ...
    async def _start_once(self) -> None:
        ...
        # before the static language menu so callers cannot enter a profile
    ...
    def _resolve_language_profile(
        ...
        if lang == "ta":
            # Tamil rides the same pipeline Kavya proved on Twilio Media
            # Streams: Google STT (ta-IN, en-US alternative) and ElevenLabs
            # eleven_multilingual_v2 via _speak(); the LLM is the primary
            # provider. Google is forced regardless of STT_PROVIDER because the
            # Azure stream in this codebase was tuned for Sinhala only.
            return SmartPBXLanguageProfile(
                lang="ta",
                stt_provider="google",
                stt_language="ta",
        ...
        if lang != "si":
            raise ValueError(f"unsupported SmartPBX language {lang!r}")
    ...
    def _require_pipeline(self) -> Any:
        if self._pipeline is None:
            raise RuntimeError("Riya media pipeline is unavailable")
