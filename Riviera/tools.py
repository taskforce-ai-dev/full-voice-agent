"""
Claude tool definitions for the Riviera Resort voice agent (Riya).
...
Riviera Resort is a SINGLE property — a lagoon-front resort on the Kallady lagoon
in Batticaloa, Sri Lanka's east coast; reservations line 065 222 2164, WhatsApp
077 842 2223. It has exactly seven room types, all distinct:

  * Family Chalet                  (up to 4 guests)
  * Basic Room Single              (up to 2 guests)
  * Wooden Cabana                  (up to 2 guests, fan-cooled, no A/C)
  * Lagoon View Steel Cabana       (up to 2 guests)
  * Double Lagoon or Garden View   (up to 3 guests)
  * Triple Garden View             (up to 3 guests)
  * Family Cottage                 (up to 5 guests)

Rates ARE published for Sri Lankan residents and the agent quotes them: LKR per
room per night, ROOM ONLY, inclusive of all taxes, in two date-range seasons
(mid: 1 Sep 2026 – 30 Jun 2027; high: 1 Jul – 31 Aug 2027). Meal plans (BB / HB)
are per-person per-day supplements. The figures live in
yanolja_service.NIGHTLY_RATE_LKR / MEAL_PLANS_LKR and come from the resort's own
rate sheet. No foreign-guest rate is published.

SINGLE-PROPERTY MODE. This module (cloned from Kavya) previously served two
...
always resolves to PROPERTY_RIVIERA. Leaving either in place would have made the
...
PROPERTIES: tuple[str, ...] = (PROPERTY_RIVIERA,)

# The seven Riviera Resort room types. All distinct, none a prefix of another, so
# a room request is never ambiguous. These strings must match
# yanolja_service.ROOM_TYPES_BY_PROPERTY and room_types.name in the PMS
# byte-for-byte — see ops/riviera-pms/seed_riviera_room_types.sql.
...
def normalise_property(value: str | None) -> str:
    ...
    Resort is the only property, so a missing or unrecognised value is not an
...
_PROPERTY_SCHEMA: dict[str, Any] = {
    ...
    "description": (
        "The property, always 'Riviera Resort'. OPTIONAL — Riviera Resort is the only "
...
TOOL_DEFINITIONS: list[dict[str, Any]] = [
    {
        "name": "check_availability",
                "description": (
            "Check room availability at Riviera Resort for a given date range. "
            "Riviera Resort is the only property, so do NOT ask the guest which "
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "rate_type": {
                    "type": "string",
                    "description": (
                        "Meal plan code. Riviera Resort rates are ROOM ONLY ('RO', "
                        "the default). Use 'BB' if the guest adds bed and breakfast "
                        "or 'HB' if they add half board (breakfast and dinner)."
                    ),
                    "default": "RO",
                },
                "salutation": {
    ...
    },
    {
        "name": "create_booking",
        "description": (
            "Create a new reservation at Riviera Resort. Use ONLY after "
            "confirming availability and getting explicit confirmation from "
            "the guest. Riviera Resort is the only property and 'property' is "
            "optional. The room_type must be one of the seven Riviera Resort room "
            "types."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "room_type": {
                    "type": "string",
                    "description": (
                        "Human-readable room type name. One of the seven Riviera "
                        "Resort room types: 'Family Chalet' (up to 4 guests), "
                        "'Basic Room Single', 'Wooden Cabana', 'Lagoon View Steel "
                        "Cabana' (each up to 2 guests), 'Double Lagoon or Garden "
                        "View', 'Triple Garden View' (each up to 3 guests), "
                        "'Family Cottage' (up to 5 guests). Use the full name exactly."
                    ),
                },
                "guest_name": {
                    "type": "string",
    ...
    },
    {
        ...
        "description": (
            "Look up an existing reservation at Riviera Resort. Use "
            "when a guest asks "
...
# during a normal call would let Riya promise a callback instead of doing the
# live transfer.
# ---------------------------------------------------------------------------
...
async def execute_tool(tool_name: str, tool_input: dict[str, Any]) -> str:
    ...
    SINGLE-PROPERTY MODE: `normalise_property` always resolves to "Riviera Resort",
    ...
    if tool_name == "check_availability":
        ...
        result = await check_availability(
            ...
            rate_type=tool_input.get("rate_type", "RO"),
