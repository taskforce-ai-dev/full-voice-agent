...
async def main():
    ...
    # 2) Filtered availability — Family Chalet with 3 adults + 1 child
    avail_luxe = await check_availability(
        check_in=ci,
        check_out=co,
        room_type="Family Chalet",
        num_adults=3,
        num_children=1,
    )
    dump("AVAIL (Family Chalet, 3A+1C — was failing before the cap fix)", avail_luxe)
