...
picks up, the caller is dropped back into Riya. This module is the failsafe:
Riya collects (or confirms) the guest's name and WhatsApp number and POSTs a
handover payload to n8n, which sends the property manager a WhatsApp message.

Payload contract (n8n `/webhook/riviera-handover`):
...
# Fail closed: no default host. A blank/missing N8N_BASE_URL means the
# handover notification is never POSTed -- Riviera must not send guest
...
def expand_spoken_repeats(raw: Any) -> str:
    ...
    conversation text. "double" is a room type at Riviera Resort, so "I'd like a
...
def spoken_number_to_digits(raw: Any) -> str:
    ...
    is a room type at Riviera Resort, so this must never run over conversation text.
...
async def send_handover_notification(
    *,
    ...
    if not is_handover_webhook_configured():
        if privacy_safe:
