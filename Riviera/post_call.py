...
Riya is the reservations agent for Riviera Resort, a single lagoon-front
resort in Kallady, Batticaloa with seven room types. The extraction
still records a `property` field (always "Riviera Resort") so the Google Sheet
...
# Extraction prompt
# ---------------------------------------------------------------------------
EXTRACTION_SYSTEM_PROMPT: str = """You are a data extraction assistant for Riviera Resort, a lagoon-front resort in Kallady, Batticaloa, on Sri Lanka's east coast. It is a SINGLE property.

Analyze a phone call transcript between the resort's reservations agent (Riya) and a caller. The call may be in English, Sinhala, Tamil, or Arabic.
...

{
  ...
  "property": "always 'Riviera Resort' — there is only one property",
...
- "property" is always "Riviera Resort". There is only one property, so never write anything else and never write null.
- The seven room types are: Family Chalet (up to 4 guests), Basic Room Single, Wooden Cabana, Lagoon View Steel Cabana (each up to 2 guests), Double Lagoon or Garden View, Triple Garden View (each up to 3 guests), Family Cottage (up to 5 guests).
- Record room_preference using one of those exact seven names, and nothing else. If the guest described a room loosely ("the container one", "the A-frame", "the cottage on the lagoon"), map it to the correct full name (Lagoon View Steel Cabana, Family Chalet, Family Cottage respectively). A plain "double room" is the Double Lagoon or Garden View.
...
- Example: "Guest asked about the Family Cottage for 4 adults from 12 to 14 October. Riya confirmed availability at twenty four thousand eight hundred rupees per night room only and the guest asked to be called back."
- Another example: "Guest enquired about the Lagoon View Steel Cabana but did not give dates."
- Write in English regardless of transcript language.
- Rates ARE quoted on these calls for Sri Lankan residents (rupees per room per night, room only, taxes included; meal plans are extra per person). You may record a rate in availability_result or summary if the agent stated one. Never invent a figure that was not said.
...
def _normalize_property_and_room(result: dict[str, Any]) -> dict[str, Any]:
    ...
    Mutates and returns ``result``. Always sets ``property`` to "Riviera Resort":
    ...
    """
    result["property"] = PROPERTY_RIVIERA
    return result
...
RETRY_PROMPT: str = """Extract these fields from the Riviera Resort call transcript as a short JSON object. Use null for missing fields.
Fields: guest_name, property (always 'Riviera Resort'), num_guests, check_in (YYYY-MM-DD), check_out (YYYY-MM-DD), room_preference, availability_result, call_outcome (booking_inquiry/general_inquiry/booking_confirmed/callback_requested/dropped), follow_up_needed (Yes/No), summary (1 sentence English).
...
def _format_extraction_transcript(full_transcript: list[dict[str, str]]) -> str:
    ...
    for entry in full_transcript:
        if entry.get("role") == UNCONFIRMED_TRANSCRIPT_ROLE:
            ...
            continue
        role = "Guest" if entry.get("role") == "user" else "System" if entry.get("role") == "system" else "Riya"
