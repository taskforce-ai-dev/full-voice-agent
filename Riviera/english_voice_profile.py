...
RIVIERA_EN_ELEVENLABS_VOICE_ID = "RIVIERA_EN_ELEVENLABS_VOICE_ID"
...
@dataclass(frozen=True)
class RivieraEnglishVoiceProfile:
    ...
    def __repr__(self) -> str:
        return (
            "RivieraEnglishVoiceProfile("
            f"env_key={RIVIERA_EN_ELEVENLABS_VOICE_ID!r}, "
...
def load_riviera_english_voice_profile(
    environment: Mapping[str, str] | None = None,
) -> RivieraEnglishVoiceProfile:
    source = os.environ if environment is None else environment
    value = source.get(RIVIERA_EN_ELEVENLABS_VOICE_ID, "")
    if not isinstance(value, str) or not value.strip():
        raise ValueError(f"{RIVIERA_EN_ELEVENLABS_VOICE_ID} must be configured")
    return RivieraEnglishVoiceProfile(voice_id=value.strip())
