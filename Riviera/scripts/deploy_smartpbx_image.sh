...
LOCK_FILE=/var/lock/riviera-smartpbx-image-deploy.lock
...
SMARTPBX_LOG_ARCHIVE_DIR="${SMARTPBX_LOG_ARCHIVE_DIR:-/var/log/riviera-smartpbx}"
...
valid_repo_digest() { [[ $1 =~ ^ghcr\.io/taskforce-ai-dev/riviera@sha256:[0-9a-f]{64}$ ]]; }
...
smartpbx_status_json() {
  ...
  printf 'header = "X-Riviera-SmartPBX-Token: %s"\n' "$token" \
    | curl --silent --show-error --fail --connect-timeout 2 --max-time 5 http://127.0.0.1:8061/smartpbx/status --config -
}
...
check_smartpbx_ready() {
  local health_json
  health_json=$(curl --silent --show-error --fail --connect-timeout 2 --max-time 5 http://127.0.0.1:8061/health) || return 1
...
# Neighbouring containers that a Riviera SmartPBX recreate must leave
# untouched. Riviera's own Twilio service is always in the set; every other
# agent on the box (kavya-voice-agent, kavya-smartpbx, flico-voice-agent, ...)
# is deliberately NOT a hard dependency of Riviera's deploy -- add one here only
# when its health is something this deploy should refuse to proceed without.
SMARTPBX_ISOLATION_NEIGHBOURS="${SMARTPBX_ISOLATION_NEIGHBOURS:-riviera-voice-agent}"

capture_isolation_baseline() {
  local service index=0
  for service in $SMARTPBX_ISOLATION_NEIGHBOURS; do
    capture_service_snapshot "$service" "NEIGHBOUR_$index" || return 1
    index=$((index + 1))
  done
}

verify_isolation_baseline() {
  local service index=0 expected_var
  for service in $SMARTPBX_ISOLATION_NEIGHBOURS; do
    expected_var="NEIGHBOUR_${index}_ID"
    [[ $(docker inspect --format '{{.Id}}' "$service") == "${!expected_var}" ]] || return 1
    [[ $(docker inspect --format '{{.State.Health.Status}}' "$service") == healthy ]] || return 1
    index=$((index + 1))
  done
}

capture_baseline() {
  ROLLBACK_IMAGE_ID=$(docker inspect --format '{{.Image}}' riviera-smartpbx) || return 1
...
# Riviera/ops/riviera-smartpbx-logrotate for retention. Takes the sha256 image id
...
archive_outgoing_logs() {
  ...
  { docker logs riviera-smartpbx 2>&1 | grep -E "$SMARTPBX_LOG_ALLOWLIST_PATTERN" >"$out"; } || true
  chmod 600 "$out" 2>/dev/null || true
  return 0
}

recreate_smartpbx() {
  SMARTPBX_IMAGE_TAG="$TAG" docker compose --env-file .env.smartpbx --profile smartpbx up -d --force-recreate --pull never riviera-smartpbx >/dev/null
}

verify_running_image() {
  local expected_id=$1 expected_digest=$2 expected_revision=$3 current_id current_digest current_revision
  current_id=$(docker inspect --format '{{.Image}}' riviera-smartpbx) || return 1
...
# or fails the deploy). Removes stale ghcr.io/taskforce-ai-dev/riviera tags,
# keeping exactly: the tag this container is now running, the Twilio
# riviera-voice-agent container's tag, rollback-local, latest, and any
...
prune_stale_riviera_images() {
  local smartpbx_ref twilio_ref image_line tag
  smartpbx_ref=$(docker inspect --format '{{.Config.Image}}' riviera-smartpbx 2>/dev/null) || smartpbx_ref=""
  twilio_ref=$(docker inspect --format '{{.Config.Image}}' riviera-voice-agent 2>/dev/null) || twilio_ref=""
