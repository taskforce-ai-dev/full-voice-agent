...
readonly PROTECTED_FILE=/opt/riviera/.env.smartpbx
readonly TRANSACTION_DIR=/opt/riviera/.smartpbx-sinhala-rollback
...
authenticated_post_deploy_checks() {
  ...
  health=$(curl --silent --show-error --fail --connect-timeout 2 --max-time 5 http://127.0.0.1:8061/health 2>/dev/null) || return 1
  jq -e '.status == "ok" and .service_mode == "smartpbx"' >/dev/null 2>&1 <<<"$health" || return 1
  status_json=$(printf 'header = "X-Riviera-SmartPBX-Token: %s"\n' "$token" | curl --silent --show-error --fail --connect-timeout 2 --max-time 5 http://127.0.0.1:8061/smartpbx/status --config - 2>/dev/null) || return 1
