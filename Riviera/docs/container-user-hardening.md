# Running the Riya container as a non-root user — do this deliberately, not casually
...
only after deploy, on the host volumes both `riviera-voice-agent` and
`riviera-smartpbx` write to.
...
change adds `RUN useradd ... && USER riviera` to the `Dockerfile` and
...
3. The legacy service crash-loops (`riviera-voice-agent` restarts, answers no
...
4. **Roll out to one service at a time.** SmartPBX (`riviera-smartpbx`) is the
   ...
   the same change to the legacy `riviera-voice-agent` service, which has no
