#!/usr/bin/env bash
# =============================================================================
# deploy.sh — Riya Voice Agent Deployment Script
# =============================================================================
# Usage:
...
DOMAIN="riviera.taskforceai.tech"
EMAIL="info@taskforceai.tech"
# ======================================================

SERVER_USER="root"
REMOTE_DIR="/opt/riviera"
...
PROD_FILES=(
  ...
  full-voice-agent-a8a245fb37cb.json
)
...
cmd_setup() {
  check_config
  info "Setting up $SERVER_IP for Riya deployment..."
...
UFW_SETUP
  ...
  run_remote "bash -s" <<NGINX_TEMP
    cat > /etc/nginx/sites-available/riviera <<'CONF'
server {
    listen 80;
    server_name ${DOMAIN};
    location / {
        return 200 'Riya setup in progress';
        add_header Content-Type text/plain;
    }
}
CONF
    ln -sf /etc/nginx/sites-available/riviera /etc/nginx/sites-enabled/
...
NGINX_TEMP
  ...
  run_remote "sed 's/YOUR_DOMAIN/${DOMAIN}/g' ${REMOTE_DIR}/nginx.conf > /etc/nginx/sites-available/riviera && nginx -t && systemctl reload nginx"
...
cmd_logs() {
  check_config
  run_remote "cd ${REMOTE_DIR} && docker compose logs -f riviera"
}
