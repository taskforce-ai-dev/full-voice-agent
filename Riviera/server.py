"""
server.py — Main FastAPI server for Riviera Resort Voice Agent (Riya).

Riviera Resort is a SINGLE property: a lagoon-front resort on the Kallady
lagoon in Batticaloa, on Sri Lanka's east coast, with exactly seven room types.
Cloned from Kavya (Hatton Hills); the two-property disambiguation machinery it
inherited stays collapsed to single-property mode — see
yanolja_service.resolve_property.
...
if os.getenv("SENTRY_DSN"):
    ...
    sentry_sdk.set_tag("agent", "riviera")
...
from english_voice_profile import load_riviera_english_voice_profile
...
# OpenAI gpt-4o-mini-tts -- Riya's Sinhala voice. Natural prosody, streams
...
OPENAI_TTS_INSTRUCTIONS: str = os.getenv(
    ...
    "You are Riya, a warm and friendly reservations agent at Riviera "
    "Resort, a lagoon-front resort in Batticaloa. Speak in natural, "
...
# Riviera Resort reservations line, rendered digit-by-digit the way Riya must
# say it aloud. Keep in sync with yanolja_service.RESERVATIONS_PHONE
# (065 222 2164) and the knowledge base.
RESERVATIONS_PHONE_WORDS: str = "zero six five, two two two, two one six four"
...
RIVIERA_SERVICE_MODE: str = os.getenv("RIVIERA_SERVICE_MODE", "twilio").strip().lower()
...
# when this is enabled -- only finalized guest turns and TTS-submitted Riya
...
# empty history, so everything Riya needs to run the failsafe (what the guest
...
# without filtering, Riya would jump in mid-thought, derailing the call.
...
IVR_MENU_ENABLED: bool = os.getenv("IVR_MENU_ENABLED", "true").lower() == "true"

# Maps DTMF digit -> language code.
# Riviera is a three-language line: 1 = English (ConversationRelay), 2 = Sinhala
# and 3 = Tamil (both Media Streams). The Arabic path inherited from Kavya stays
# coded but unlisted. To remove a language, drop its digit here, its <Say>
# prompt in /voice/incoming, and its entry in the ws_media_stream guard.
...
#      digit words. Without these, "double seven" is garbled and Riya never
...
_DEFAULT_EN_HINTS = (
    ...
    "seven, eight, nine, "
    "Chanya, Shehani, Oshadi, Riya, Nimal, Kamal, Sunil, Saman, Chaminda, "
...
LANGUAGE_CONFIGS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "hints": CR_HINTS_EN,
        "welcome_greeting": "Welcome to Riviera Resort! I'm Riya, how can I help you today?",
...
def conversation_relay_config(language: str) -> dict[str, str]:
    ...
    if language == "en":
        config["voice"] = load_riviera_english_voice_profile().twilio_composite_voice
    return config
...
def _log_smartpbx_pilot_transcript(role: str, phrase: str) -> None:
    ...
    if not SMARTPBX_PILOT_TRANSCRIPT_LOGGING:
        return
    if role not in {"guest", "riviera"} or not isinstance(phrase, str) or not phrase:
        return
...
_STT_BOOKING_TERMS: tuple[str, ...] = (
    "Riviera Resort", "check-in", "check in", "check-out", "check out",
    "honeymoon", "anniversary", "half board", "full board", "bed and breakfast",
    "adults", "children", "child", "guests", "nights", "double room",
    "room only", "lagoon view", "garden view", "cabana", "cottage", "chalet",
    "air conditioned", "twin beds", "extra bed", "Kallady", "Batticaloa",
    "breakfast", "dinner", "availability", "reservation", "booking",
)
EN_STT_PHRASE_LIST: tuple[str, ...] = (
    tuple(ROOM_TYPES_BY_PROPERTY[PROPERTY_RIVIERA])
    + _STT_BOOKING_TERMS
    + _STT_DIGIT_WORDS
    + _STT_REPEAT_PHRASES
)
...
# DTMF keypad number capture. Keypad entry bypasses STT and is exact, so Riya
...
#
# These patterns arm capture mode from Riya's own ask, matched ONLY against
...
# matches the first pattern above, and re-arming capture mode on Riya's own
...
# the seven Riviera Resort room types. Biasing the recognizer toward the room
...
_SI_ROOM_NAME_PHRASES: tuple[str, ...] = (
    *ROOM_TYPES_BY_PROPERTY[PROPERTY_RIVIERA],
    "Family", "Chalet", "Basic", "Room", "Single", "Wooden", "Cabana",
    "Lagoon", "View", "Steel", "Double", "Garden", "Triple", "Cottage",
    "ෆැමිලි", "චලට්", "ෂැලේ", "බේසික්", "රූම්", "සිංගල්", "වුඩන්", "කබානා",
    "ලගූන්", "වීව්", "ස්ටීල්", "ඩබල්", "ගාඩ්න්", "ට්‍රිපල්", "කොටේජ්",
)
...
# Barge-in threshold. While Riya is speaking, only a SUBSTANTIVE interruption
...
REPROMPT_MESSAGES: dict[str, list[str]] = {
    "en": [  # English
        ...
        "Welcome to Riviera Resort. How may I help you today?",
    ],
    "ar": [  # Arabic (MSA)
        "\u0645\u0631\u062d\u0628\u0627\u064b\u060c \u0647\u0644 \u0645\u0627 \u0632\u0644\u062a\u0645 \u0639\u0644\u0649 \u0627\u0644\u062e\u0637\u061f",
        "\u0623\u0647\u0644\u0627\u064b \u0628\u0643\u0645 \u0641\u064a Riviera Resort. \u0643\u064a\u0641 \u064a\u0645\u0643\u0646\u0646\u064a \u0645\u0633\u0627\u0639\u062f\u062a\u0643\u0645 \u0627\u0644\u064a\u0648\u0645\u061f",
    ],
    "si": [  # Sinhala
        "\u0d86\u0dba\u0dd4\u0db6\u0ddd\u0dc0\u0db1\u0dca, \u0d94\u0db6 \u0dad\u0dc0\u0db8\u0dad\u0dca \u0dc3\u0dd2\u0da7\u0dd2\u0db1\u0dca\u0db1\u0dda\u0daf?",
        "\u0d86\u0dba\u0dd4\u0db6\u0ddd\u0dc0\u0db1\u0dca! Riviera Resort \u0dc0\u0dd9\u0dad \u0dc3\u0dcf\u0daf\u0dbb\u0dba\u0dd9\u0db1\u0dca \u0db4\u0dd2\u0dc5\u0dd2\u0d9c\u0db1\u0dd2\u0db8\u0dd4. \u0db8\u0da7 \u0d94\u0db6\u0da7 \u0d9a\u0dd9\u0dc3\u0dda \u0d8b\u0daf\u0dc0\u0dca \u0d9a\u0dc5 \u0dc4\u0dd0\u0d9a\u0dd2\u0daf?",
    ],
    "ta": [  # Tamil
        "\u0bb5\u0ba3\u0b95\u0bcd\u0b95\u0bae\u0bcd, \u0ba8\u0bc0\u0b99\u0bcd\u0b95\u0bb3\u0bcd \u0b87\u0ba9\u0bcd\u0ba9\u0bc1\u0bae\u0bcd \u0b87\u0bb0\u0bc1\u0b95\u0bcd\u0b95\u0bbf\u0bb1\u0bc0\u0bb0\u0bcd\u0b95\u0bb3\u0bbe?",
        "\u0bb5\u0ba3\u0b95\u0bcd\u0b95\u0bae\u0bcd! Riviera Resort \u0b95\u0bcd\u0b95\u0bc1 \u0bb5\u0bb0\u0bb5\u0bc7\u0bb1\u0bcd\u0b95\u0bbf\u0bb1\u0bcb\u0bae\u0bcd. \u0ba8\u0bbe\u0ba9\u0bcd \u0b89\u0b99\u0bcd\u0b95\u0bb3\u0bc1\u0b95\u0bcd\u0b95\u0bc1 \u0b8e\u0baa\u0bcd\u0baa\u0b9f\u0bbf \u0b89\u0ba4\u0bb5\u0bb2\u0bbe\u0bae\u0bcd?",
    ],
}
...
MEDIA_STREAM_WELCOME: dict[str, str] = {
    "ar": (
        "أهلاً وسهلاً بكم في Riviera Resort! "
        "أنا ريا، كيف يمكنني مساعدتكم اليوم؟"
    ),
    "si": (
        "\u0D86\u0DBA\u0DD4\u0DB6\u0DDD\u0DC0\u0DB1\u0DCA! "
        "Riviera Resort \u0DC0\u0DD9\u0DAD "
...
def _build_system_prompt(lang: str = "en") -> str:
    ...
    if lang == "si":
        language_rules = (
            ...
            "- Natural English code-switching is allowed for official room names, Riviera "
            "Resort, WhatsApp, and familiar hotel terms. Never romanize Sinhala words.\n"
            ...
            "speech and STT can mishear them. Likely matches: කබානා = Cabana "
            "(Wooden Cabana, Lagoon View Steel Cabana); ෂැලේ/චලට් = Chalet (Family "
            "Chalet); කොටේජ් = Cottage (Family Cottage); ලගූන් = Lagoon; ගාඩ්න් = "
            "Garden; ස්ටීල් = Steel; වුඩන් = Wooden; ඩබල් = Double (Double Lagoon or "
            "Garden View); ට්‍රිපල් = Triple (Triple Garden View); බේසික් = Basic "
            "(Basic Room Single). If the room name is unclear, confirm it by name "
            "instead of guessing, e.g. ask if they meant the Lagoon View Steel Cabana.\n\n"
        )
    elif lang == "ta":
        language_rules = (
            "LANGUAGE RULES:\n"
    ...
    if lang == "en":
        handoff_rules = (
            ...
            "- If you do NOT know the answer to a guest's question, or the request is outside what you can help with (e.g. complex booking changes, special packages, complaints, anything not covered by Riviera Resort booking/general info), PROACTIVELY offer transfer and take the next obvious handoff step.\n"
    ...
    # Rates. Two mutually exclusive regimes, switched by RATES_ENABLED (legacy
    # alias DEMO_RATES_ENABLED; see yanolja_service's RATE CARD block). The
    # default-on regime lets Riya quote the resort's resident rate sheet; the
    # off regime is the original "we publish no rates" behaviour. The KB
    # carries matching wording, so flip both together — the env var is the
    # kill switch.
    if yanolja_service.DEMO_RATES_ENABLED:
        rates_rules = (
            "RATES AND PRICING:\n"
            "- Riviera Resort publishes rates for SRI LANKAN RESIDENTS only, in "
            "rupees, per room per night, ROOM ONLY, inclusive of all taxes. There "
            "are two rate seasons: mid season from the first of September 2026 to "
            "the thirtieth of June 2027, and high season from the first of July "
            "2027 to the thirty first of August 2027. Always match the rate to "
            "the guest's dates.\n"
            "- Every room-rate sentence must say 'room only', 'per room per "
            "night', 'rupees', and that it is inclusive of all taxes. For "
            "example: 'the Lagoon View Steel Cabana is fourteen thousand six "
            "hundred rupees per room per night, room only, inclusive of all "
            "taxes'.\n"
            "- Meals are NOT included in the room rate. If the guest wants "
            "meals, add bed and breakfast at two thousand rupees per adult per "
            "day, or half board with breakfast and dinner at four thousand five "
            "hundred rupees per adult per day; children aged three to twelve pay "
            "one thousand five hundred rupees for bed and breakfast or three "
            "thousand rupees for half board per day. State the meal plan as a "
            "separate per-person amount, never folded into the room rate.\n"
            "- The room rate you quote is the FINAL room rate. Never add a tax or "
            "service charge on top of it, never say 'plus taxes', and never imply "
            "the guest will pay more for the room than the figure you gave.\n"
            "- You MAY quote the rates given in the hotel information in "
            "context, the AUTHORITATIVE RATE RECORD when one is present, and the "
            "rate returned by the check_availability tool.\n"
            "- FOREIGN GUESTS: there is no published foreign-guest rate. Never "
            "quote the resident rupee figures to a foreign guest and never convert "
            "them to another currency. Say the reservations team will confirm the "
            "rate for their dates and give the reservations number: "
            f"{RESERVATIONS_PHONE_WORDS}.\n"
            "- If a stay straddles the two seasons, or falls outside the "
            "published rate periods, do not blend or guess a rate — say "
            "reservations will confirm the total for those dates.\n"
            ...
            "- NEVER invent a rate for anything not priced in your context. Extra "
            "bed and extra child supplements, activity prices, transfers and "
            "packages are not priced here: say you do not have that figure to "
            f"hand and offer the reservations number: {RESERVATIONS_PHONE_WORDS}.\n"
        ...
        rates_rules = (
            ...
            "- Riviera Resort rate quoting is disabled in this configuration. Rates "
            "exist only once "
            ...
            "team will confirm them, and give the reservations number: "
            f"{RESERVATIONS_PHONE_WORDS}. "
    ...
    if yanolja_service.DEMO_RATES_ENABLED:
        ...
        avail_price_clause = (
            "Whenever you do name a room for a Sri Lankan resident, give its "
            "nightly rate with it in rupees, room only, inclusive of all taxes, "
            "per room per night, for the season their dates fall in. For a "
            "foreign guest name the room without a figure and say reservations "
            "will confirm the rate.\n"
        )
        rate_press_clause = (
            "- If the guest asks for a total, multiply the room-only nightly "
            "rate by the number of nights and state it plainly; add the meal "
            "plan per person per day separately if they asked for meals. For "
            f"anything you have no figure for, offer reservations on {RESERVATIONS_PHONE_WORDS}.\n"
        )
    else:
        tool_rate_clause = (
        ...
        rate_press_clause = (
            ...
            "reservations on zero six five, two two two, two one six four.\n"
        )
    ...
    if lang == "en":
        greeting_note = (
            "The caller has already heard your greeting: 'Welcome to Riviera "
            "Resort! I'm Riya, how can I help you today?' — do NOT "
    ...
    return (
        f"You are Riya, the warm and gracious reservations voice agent for "
        f"Riviera Resort, a lagoon-front resort on the calm Kallady lagoon in "
        f"Batticaloa, on Sri Lanka's east coast, set in lush tropical gardens "
        f"with lagoon views and sunsets over the water.\n"
        f"Riviera Resort is a SINGLE property. There is no second hotel, no other "
        f"branch and no other location, so never ask the caller which property "
        f"or which location they mean.\n"
        f"It has exactly seven room types: the Family Chalet sleeps up to four; "
        f"the Basic Room Single, the Wooden Cabana and the Lagoon View Steel "
        f"Cabana each sleep up to two; the Double Lagoon or Garden View and the "
        f"Triple Garden View each sleep up to three; and the Family Cottage "
        f"sleeps up to five. Every room is air conditioned except the Wooden "
        f"Cabana and the Basic Room Single, which are fan-cooled.\n"
        f"Riviera Resort is a family-run, eco-friendly resort established in "
        f"1981 on ten acres of lagoon-front gardens in Kallady, five minutes "
        f"from Batticaloa town and a ten-minute walk from Kallady Beach, with "
        f"an infinity-style pool, a lagoon-front restaurant and a bar.\n"
        f"Rates are room only; breakfast or half board can be added per person.\n"
        f"Reservations line: zero six five, two two two, two one six four. WhatsApp: zero seven seven, eight "
        f"four two, two two two three.\n"
        ...
        "THE ROOM TYPES AT RIVIERA RESORT:\n"
        ...
        "- The room vocabulary is fixed and there are exactly seven:\n"
        "    * Family Chalet - up to four guests; modern A-frame chalet, "
        "ground-floor bedroom plus a loft with two single beds, lagoon views, "
        "air conditioned.\n"
        "    * Basic Room Single - up to two guests; the simplest, most "
        "affordable room, compact, fan-cooled (NO air conditioning). Say so "
        "before booking it.\n"
        "    * Wooden Cabana - up to two guests; traditional wooden cabana on "
        "stilts with a lagoon view. NOT air conditioned - ceiling fan and sea "
        "breeze. Always say so before booking it.\n"
        "    * Lagoon View Steel Cabana - up to two guests; modern cabana "
        "converted from a shipping container, on stilts, lagoon view, air "
        "conditioned.\n"
        "    * Double Lagoon or Garden View - up to three guests; classic double "
        "room, one double bed or two singles, extra bed on request, lagoon or "
        "garden view, air conditioned.\n"
        "    * Triple Garden View - up to three guests; first-floor room with one "
        "double and one single bed or three singles, garden view, air "
        "conditioned.\n"
        "    * Family Cottage - up to five guests; standalone cottage right on "
        "the lagoon front, one double and three single beds, the most private "
        "unit, air conditioned.\n"
        "- Always use the FULL room name when you confirm or book, for example "
        "'the Lagoon View Steel Cabana', not 'the cabana'. Booking needs the "
        "exact name. If a guest just says 'cabana', ask whether they mean the "
        "Wooden Cabana or the Lagoon View Steel Cabana.\n"
        "- If a guest asks for a room type that does not exist here, say so "
        "plainly and offer the closest of the seven. Never invent a room type.\n"
        "- If a guest asks for a 'double room', that is the Double Lagoon or "
        "Garden View. For two guests wanting a lagoon view, recommend the "
        "Lagoon View Steel Cabana or a Double Lagoon View room. For four "
        "guests recommend the Family Chalet; for five, the Family Cottage.\n\n"
        ...
        "you are an AI reservations agent for Riviera Resort, built by TaskForce "
        ...
        f"{tool_rate_clause} "
        "from the tool. Only ever use the seven Riviera Resort room names: "
        "Family Chalet, Basic Room Single, Wooden Cabana, Lagoon View Steel "
        "Cabana, Double Lagoon or Garden View, Triple Garden View and Family "
        "Cottage.\n"
        ...
        "is free and move straight on to booking, e.g. 'The Family Cottage "
        "is available for those dates.' Do NOT re-list the other room "
        ...
        "type in one sentence, e.g. 'The Wooden Cabana and the Lagoon View "
        "Steel Cabana are available for those dates — which would "
        "you prefer?' "
        ...
        "call. Sri Lankan resident = the rupee rate sheet; foreign guest = no "
        "published rate, reservations will confirm.\n"
        "- ACTIVITIES: if the guest asks what there is to do, mention once that "
        "the resort offers kayaking on the lagoon, cycling, a lagoon cruise by "
        "boat and the singing fish boat ride, plus fishing, a pool with a kids "
        "pool and jacuzzi, and a children's play area, and that the resort "
        "confirms activity prices. Do not invent an activity price.\n"
        + rate_press_clause +
        ...
        "with 'shall I go ahead and book the Family Cottage "
        ...
        "- STAY BASICS you may state plainly: check-in is from two in the "
        "afternoon and check-out is by twelve noon; the resort is open "
        "twenty-four hours. Early arrival is guaranteed only by reserving the "
        "night before. Late departure is a courtesy subject to availability: "
        "free until one in the afternoon, half the room rate until four, and "
        "the full room rate after four (the room is then held until noon the "
        "next day). Offer to note an early or late request on the booking.\n"
        "- DEPOSIT AND CANCELLATION: every reservation is confirmed only once a "
        "deposit of at least one night is paid; special dates such as holidays "
        "and events may need full non-refundable prepayment. Cancel at least "
        "forty-eight hours before arrival; a cancellation inside forty-eight "
        "hours is charged one night's room and tax. Payment can be made by "
        "Visa, American Express, MasterCard, bank transfer or QR pay. After "
        "create_booking succeeds, tell the guest the reservations team will "
        "contact them for the one-night deposit that confirms the booking. "
        "State these terms as words, never as an invented amount of money.\n"
        ...
        "you're ready to make a booking. Thank you for calling Riviera "
        "Resort.' Then stop. Do not keep repeating the question.\n"
...
async def _notify_handover_fallback(
    ...
    * the failsafe session's `finally`, if the guest hung up before Riya got
...
# dropped back into Riya. It must set up the details request immediately —
...
def _build_handoff_failsafe_prompt(state: dict) -> str:
    ...
    Riya has exactly one job here: get a name and a WhatsApp number, send them
    ...
    return (
        f"You are Riya, the warm and gracious reservations voice agent for "
        f"Riviera Resort, a lagoon-front resort in Kallady, Batticaloa, on "
        f"Sri Lanka's east coast.\n"
        f"Today's date is {today}.\n\n"
...
def _claude_tools_from_gemini(tools: Any) -> list[dict[str, Any]]:
    ...
    ``notify_human_handover``, and handing it the full booking set would let Riya
...
async def lifespan(app: FastAPI):
    ...
    # --- Startup ---
    logger.info("Starting Riviera Resort Voice Agent server...")
    ...
    if RIVIERA_SERVICE_MODE != "smartpbx":
    ...
    # yanolja_client. The PMS instance Riviera points at (YANOLJA_BASE_URL) IS
    # the source of truth for availability and bookings (see ops/riviera-pms/).
...
# ---------------------------------------------------------------------------
app = FastAPI(
    title="Riviera Resort Voice Agent (Riya)",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def kb_reload(request: Request) -> dict:
    ...
    filename: str = body.get("filename", "riviera_info.txt")
    if not content:
...
async def voice_incoming(request: Request) -> Response:
    ...
    # IVR language menu (IVR_MENU_ENABLED=true, the Riviera default): 1 = English
    # (ConversationRelay), 2 = Sinhala, 3 = Tamil (both Media Streams). Any
    # other digit or no input falls back to English. With the menu disabled the
    # <Gather> is omitted entirely and every call connects straight to the
    # English agent below.
    gather = ""
    if IVR_MENU_ENABLED:
        gather = (
            ...
            '    <Say voice="Polly.Joanna">Welcome to Riviera Resort. '
            'For English, press 1.</Say>\n'
            '    <Say voice="Google.si-LK-Standard-A" language="si-LK">'
            '\u0dc3\u0dd2\u0d82\u0dc4\u0dbd \u0dc3\u0da0\u0dc4\u0dcf, '
            '\u0daf\u0dd9\u0d9a \u0d94\u0db6\u0db1\u0dca\u0db1.</Say>\n'
            '    <Say voice="Google.ta-IN-Standard-A" language="ta-IN">'
            '\u0ba4\u0bae\u0bbf\u0bb4\u0bc1\u0b95\u0bcd\u0b95\u0bc1, '
            '\u0bae\u0bc2\u0ba9\u0bcd\u0bb1\u0bc8 '
            '\u0b85\u0bb4\u0bc1\u0ba4\u0bcd\u0ba4\u0bb5\u0bc1\u0bae\u0bcd.</Say>\n'
            "  </Gather>\n"
...
async def voice_language_selected(request: Request) -> Response:
    ...
    Sinhala (2) and Tamil (3) -> Media Streams TwiML (Google STT + per-language
    TTS). Any other digit falls back to English.
    """
    form = await request.form()
...
async def dial_result(request: Request) -> Response:
    ...
    Otherwise, drop the caller back into Riya with a recovery greeting.
    """
    form = await request.form()
    ...
    # No answer / busy / failed / canceled â†’ the failsafe. Recover into Riya
    ...
    # instead of ringing, and hangs up before Riya comes back. Live on
...
class MediaStreamSession:
    ...
    def __init__(
        ...
        # Set only by RivieraSmartPBXSession. None preserves legacy Twilio tools.
        ...
        # Set by RivieraSmartPBXSession before the first turn can begin; random
    ...
    # Riya forget details the guest already gave (mid-call slot amnesia).
    ...
    def _smartpbx_rhythm_rule(self) -> str:
        return (
            ...
            "Riviera Resort only when the caller asks.\n"
    ...
    async def _speak(self, text: str, generation: int = -1, sentence: str | None = None):
        ...
        async with self._speak_lock:
            ...
            if self._is_smartpbx_session():
                _log_smartpbx_pilot_transcript("riviera", text)
    ...
    async def _tts_elevenlabs(
        ...
        *,
        ...
        if self.lang == "en":
            try:
                profile = load_riviera_english_voice_profile()
            except ValueError:
                self._emit_smartpbx_tts_diagnostic(DiagnosticFailureClass.TTS_PROFILE_FAILURE)
                logger.warning("Canonical Riya English voice is not configured — skipping TTS")
                return
...
# The synthetic turn that makes Riya OPEN the failsafe conversation instead of
...
# Riya asks anything. Observed live on 2026-07-31: 13 s of dead air.
#
...
async def ws_conversation(websocket: WebSocket, lang: str = "en", mode: str = ""):
    ...
    human agent failed to answer a transferred call: Riya drops the booking
    ...
    if is_failsafe:
        ...
        # reach would let Riya wander back into the booking flow instead of
    ...
    try:
        while True:
            ...
            if msg_type == "setup":
                ...
                if is_failsafe:
                    # Rebuild Riya's context from the pre-transfer leg of this
            ...
            # ---------------------------------------------------------------
                ...
                # failsafe session — Riya is only taking a name and a phone
        ...
        if is_failsafe:
            ...
            # before Riya could send the details. Notify the manager anyway
...
async def ws_media_stream(websocket: WebSocket, lang: str):
    ...
    Riviera serves Sinhala ("si") and Tamil ("ta") on this path. The Arabic
    ("ar") path inherited from Kavya is coded but not offered on the IVR, so it
    refuses the connection; add it here (and a digit) to enable it.
    """
...
async def _new_smartpbx_session(context, transport, diagnostic_sink=None):
    ...
    return RivieraSmartPBXSession(context, transport, diagnostic_sink=diagnostic_sink)
...
def build_service_app(
    ...
    if mode == "twilio":
        return _twilio_app
    if mode != "smartpbx":
        raise ValueError("invalid RIVIERA_SERVICE_MODE")
    ...
    smartpbx_app = FastAPI(
        title="Riviera Resort Voice Agent (Riya) — SmartPBX",
        version="1.0.0",
        lifespan=lifespan,
        docs_url=None,
        redoc_url=None,
...
app = build_service_app(RIVIERA_SERVICE_MODE, os.environ)
