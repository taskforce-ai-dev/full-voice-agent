# Riya Dialog SmartPBX cutover runbook

This runbook operates only the opt-in `riviera-smartpbx` profile at `/opt/riviera`.
It leaves the existing `riviera` Twilio service unchanged and keeps Flico untouched:
...
SMARTPBX_IMAGE="ghcr.io/taskforce-ai-dev/riviera:$REVIEWED_CI_SHORT_SHA"
...
cd /opt/riviera
umask 077
touch /opt/riviera/.env.smartpbx
chmod 600 /opt/riviera/.env.smartpbx
openssl rand -hex 32
```
...
ELEVENLABS_API_KEY=
RIVIERA_EN_ELEVENLABS_VOICE_ID=
ELEVENLABS_VOICE_ID=
...
N8N_HANDOVER_WEBHOOK=
DASHBOARD_API_URL=
DASHBOARD_API_KEY=
DASHBOARD_AGENT_ID=riviera
SENTRY_DSN=
...
The SmartPBX call menu is **1 English, 2 Sinhala, 3 Tamil**. The timeout
defaults to English; an invalid selection replays once then defaults to
English. This is a direct SmartPBX-only choice: the Twilio line has its own
three-language `<Gather>` menu (`IVR_MENU_ENABLED`).
...
`gemini-3.1-flash-tts-preview` / `Vindemiatrix`. Press 3 uses Google `ta-IN`
STT (with `en-US` as the code-switch alternative) -> the primary LLM
(`LLM_PROVIDER`/`CLAUDE_MODEL`) -> ElevenLabs `eleven_multilingual_v2` on
`ELEVENLABS_VOICE_ID`, i.e. the Tamil pipeline Kavya already proved on Twilio
Media Streams. The trilingual prompt before selection is the reviewed static
μ-law asset documented below; no menu segment makes a live TTS request.
...
nonblank `GEMINI_API_KEY` is required before exposing the language menu,
including to a caller who will press `1` or `3`. Blank, whitespace-only, duplicate,
...
the locked `/opt/riviera/.smartpbx-sinhala-rollback` directory (already
...
procedure, the reviewed `/opt/riviera` checkout must contain this utility and it
...
set -euo pipefail
cd /opt/riviera
if ! sudo /opt/riviera/scripts/update_smartpbx_sinhala_provider.sh apply /opt/riviera/.env.smartpbx claude; then
  return 1
fi
...
if ! SMARTPBX_IMAGE_TAG="$REVIEWED_CI_SHORT_SHA" docker compose --env-file .env.smartpbx --profile smartpbx config > /dev/null; then
  sudo /opt/riviera/scripts/update_smartpbx_sinhala_provider.sh restore /opt/riviera/.env.smartpbx
  sudo /opt/riviera/scripts/update_smartpbx_sinhala_provider.sh cleanup /opt/riviera/.env.smartpbx
  return 1
fi
if ! sudo /opt/riviera/scripts/deploy_smartpbx_image.sh "$REVIEWED_CI_SHORT_SHA" "$REVIEWED_FULL_COMMIT_SHA" "$REVIEWED_IMAGE_DIGEST"; then
  sudo /opt/riviera/scripts/update_smartpbx_sinhala_provider.sh restore /opt/riviera/.env.smartpbx
  sudo /opt/riviera/scripts/deploy_smartpbx_image.sh "$REVIEWED_CI_SHORT_SHA" "$REVIEWED_FULL_COMMIT_SHA" "$REVIEWED_IMAGE_DIGEST"
fi
sudo /opt/riviera/scripts/update_smartpbx_sinhala_provider.sh cleanup /opt/riviera/.env.smartpbx
}
rollback_smartpbx_sinhala_llm
...
After the guarded deployment, run this **three-language canary call checklist**:

1. Call the SmartPBX endpoint, press `1`, and confirm the English pipeline
   (canonical female British ElevenLabs voice, `RIVIERA_EN_ELEVENLABS_VOICE_ID`).
2. Call again, press `2`, and confirm Azure `si-LK`, Gemini LLM, and Gemini
   `Vindemiatrix` TTS respond in Sinhala.
3. Call again, press `3`, and confirm Google `ta-IN` STT, the primary LLM, and
   ElevenLabs `eleven_multilingual_v2` (`ELEVENLABS_VOICE_ID`) respond in Tamil.
4. Confirm `/health` and authenticated `/smartpbx/status`; inspect only
   bounded provider/event/outcome diagnostics if any call fails.
...
`/opt/riviera/.env.smartpbx` file. Keep `RIME_API_KEY` server-side only; do not
...
`riviera-smartpbx` with the same pinned image). Verify health, status, and the
three-language checklist before restoring traffic. Do not run a direct
unreviewed recreate or alter the Twilio `riviera` service.
...
voice saying “Welcome to Riviera Resort. For English, press 1.”, the approved
Gemini `Vindemiatrix` voice saying “සිංහල සඳහා, 2 ඔබන්න.”, and the approved
ElevenLabs multilingual voice saying “தமிழுக்கு, மூன்றை அழுத்தவும்.”

> **Asset status (clone note):** the committed asset was copied from Kavya and
> still announces only English (1) and Sinhala (2). Digit 3 already activates
> Tamil in the runtime, but callers are not told so until the asset is
> regenerated with `python3 scripts/generate_smartpbx_language_menu.py` (needs
> the ElevenLabs and Gemini credentials from the protected env; run it off-box
> and commit the result) and shipped in a new image. Do this before the first
> Tamil canary call.
...
before Riya's welcome greeting. It is default-off (`0`). The approved
...
Riya speaks and test whether that avoids startup crackle.

This is not an Opus migration. Keep the Dialog dashboard and Riya media
...
Riya must read back what she understood and receive an explicit yes/no
...
SMARTPBX_IMAGE_TAG="$REVIEWED_CI_SHORT_SHA" docker compose --env-file .env.smartpbx --profile smartpbx config --format json \
  | jq -e '.services["riviera-smartpbx"].environment | has("STT_DIGIT_CLASS_BOOST")' >/dev/null
```
...
```sh
docker compose --env-file .env.smartpbx --profile smartpbx logs --since 10m riviera-smartpbx \
...
Riya accepts `SMARTPBX_MCP_ACCOUNT_HEADER=account_id` and
...
- `SMARTPBX_LLM_INITIAL_RESPONSE_TIMEOUT_SECONDS` (default `8.0`, clamp
  ...
  deltas before Riya gives up on it.
...
already used its one same-provider retry, Riya cancels only that turn's own
...
| Sample rate | `8000` Hz |
| Media WebSocket URL | `wss://smartpbx-riviera.taskforceai.tech/ws/v1/smartpbx/media` |
| WebSocket headers | `X-Riviera-SmartPBX-Token: <SMARTPBX_WS_TOKEN>` |
...
Dialog `tools/list` contract used by Riya requires the literal MCP argument
key `destination number`. Riya sends the configured `tel:` or `sip:` URI under
...
Riya does not expose `hangup_call`. The PDF requires
...
set -euo pipefail
cd /opt/riviera
getent ahostsv4 smartpbx-riviera.taskforceai.tech | grep -F '67.207.90.109'
sudo install -m 0644 nginx-smartpbx-acme.conf /etc/nginx/sites-available/riviera-smartpbx
sudo ln -sfn /etc/nginx/sites-available/riviera-smartpbx /etc/nginx/sites-enabled/riviera-smartpbx
sudo nginx -t
sudo systemctl reload nginx
sudo certbot certonly --webroot -w /var/www/html -d smartpbx-riviera.taskforceai.tech
test -s /etc/letsencrypt/live/smartpbx-riviera.taskforceai.tech/fullchain.pem
test -s /etc/letsencrypt/live/smartpbx-riviera.taskforceai.tech/privkey.pem
SMARTPBX_IMAGE_TAG="$REVIEWED_CI_SHORT_SHA" docker compose --env-file .env.smartpbx --profile smartpbx config > /dev/null
wait_for_smartpbx_idle() {
  ...
  token=$(sed -n 's/^SMARTPBX_WS_TOKEN=//p' /opt/riviera/.env.smartpbx | head -n 1)
  deadline=$((SECONDS + 600))
  while :; do
    status_json=$(printf 'header = "X-Riviera-SmartPBX-Token: %s"\n' "$token" \
      | curl --silent --show-error --fail --connect-timeout 2 --max-time 5 http://127.0.0.1:8061/smartpbx/status --config -) || status_json=""
...
wait_for_smartpbx_idle
SMARTPBX_IMAGE_TAG="$REVIEWED_CI_SHORT_SHA" docker compose --env-file .env.smartpbx --profile smartpbx up -d --force-recreate --pull never riviera-smartpbx
smartpbx_status_token() {
  ...
  sed -n 's/^SMARTPBX_WS_TOKEN=//p' /opt/riviera/.env.smartpbx | head -n 1
}
wait_for_smartpbx_ready() {
  deadline=$((SECONDS + 90))
  while ! curl --silent --show-error --fail --connect-timeout 2 --max-time 5 http://127.0.0.1:8061/health >/dev/null \
    || ! printf 'header = "X-Riviera-SmartPBX-Token: %s"\n' "$(smartpbx_status_token)" \
      | curl --silent --show-error --fail --connect-timeout 2 --max-time 5 http://127.0.0.1:8061/smartpbx/status --config - >/dev/null; do
    if (( SECONDS >= deadline )); then
...
wait_for_smartpbx_ready
sudo install -m 0644 nginx-smartpbx.conf /etc/nginx/sites-available/riviera-smartpbx
sudo nginx -t
sudo systemctl reload nginx
curl --fail https://smartpbx-riviera.taskforceai.tech/health
printf 'header = "X-Riviera-SmartPBX-Token: %s"\n' "$(smartpbx_status_token)" \
  | curl --fail https://smartpbx-riviera.taskforceai.tech/smartpbx/status --config -
```
...
smartpbx_pilot_transcript role=riviera text='exact phrase submitted to TTS'
```

Render `docker compose --env-file .env.smartpbx --profile smartpbx config`, then
recreate only `riviera-smartpbx` with the same pinned image. Verify image identity,
health, and Flico/legacy Riya isolation before placing the controlled call. Tail
only the dedicated class locally:

```sh
docker logs --since 2m -f riviera-smartpbx 2>&1 \
...
6. **Live barge-in.** Also confirm Dialog's reconnect behaviour here: nginx now
   ...
   Ask Riya something that produces a long answer, then
...
wait_for_smartpbx_ready
printf 'header = "X-Riviera-SmartPBX-Token: %s"\n' "$(smartpbx_status_token)" \
  | curl --fail http://127.0.0.1:8061/smartpbx/status --config - | jq -e '.transfer_enabled == false'
```

## Monitoring `/smartpbx/status`

`/smartpbx/status` requires the same `X-Riviera-SmartPBX-Token` header as the media
...
printf 'header = "X-Riviera-SmartPBX-Token: %s"\n' "$(smartpbx_status_token)" \
  | curl --fail --silent http://127.0.0.1:8061/smartpbx/status --config - \
...
rising counter as "Riya is answering at unusual length" and look at the prompt
...
Twilio `riviera` service). Each file is named by a sha256 hash of
...
`/var/log/riviera-smartpbx/<utc-timestamp>-<old-image-id>.log` (mode `0600`,
...
sudo install -m 644 -o root -g root \
  /opt/riviera/ops/riviera-smartpbx-logrotate /etc/logrotate.d/riviera-smartpbx
```
...
3. Only after drain completes:
   ...
   set -euo pipefail
   cd /opt/riviera
   SMARTPBX_IMAGE_TAG="$REVIEWED_CI_SHORT_SHA" docker compose --env-file .env.smartpbx --profile smartpbx stop riviera-smartpbx
   ```
...
Prerequisites: run as root on the target host, keep `/opt/riviera/.env` and
`/opt/riviera/.env.smartpbx` owned by `root:root` with mode `0600`, retain a
healthy `riviera-voice-agent` (the default `SMARTPBX_ISOLATION_NEIGHBOURS`;
Riviera never depends on another agent's container), and ensure the reviewed
...
retention" below), and recreates only `riviera-smartpbx`. The **post-recreate
...
/opt/riviera/scripts/deploy_smartpbx_image.sh "$NEW_TAG" "$EXPECTED_SHA" "$EXPECTED_DIGEST"
```

It rolls back once for ordinary errors and `INT`, `TERM`, or `HUP`, and verifies
the restored image identity and the unchanged healthy Flico and legacy Riya
...
and never failing the deploy, removes stale `ghcr.io/taskforce-ai-dev/riviera`
tags -- keeping exactly the tag now running as `riviera-smartpbx`, the tag
running as the Twilio `riviera-voice-agent`, `rollback-local`, `latest`, and any
...
The generic `deploy.yml` rejects `agent=riviera` for every mode (image, fast,
build), so `/opt/riviera`'s host-side control plane — `docker-compose.yml`, the
...
and every `scripts/*.sh` file under `/opt/riviera` against it with `sha256sum -c`
...
Produce the manifest **from the reviewed checkout**, not from `/opt/riviera`
...
manifest=/opt/riviera/host-files.sha256   # root-owned, mode 0600, outside the repo tree
...
for f in docker-compose.yml nginx-smartpbx.conf nginx-smartpbx-acme.conf \
  ...
  hash=$(git -C "$checkout" show "$sha:Riviera/$f" | sha256sum | cut -d' ' -f1)
  printf '%s  %s\n' "$hash" "$f" >> "$manifest"
done

sudo install -o root -g root -m 0600 "$manifest" /opt/riviera/host-files.sha256
```

List every `scripts/*.sh` file that exists in the reviewed checkout under
`Riviera/scripts/` — the check fails closed if any current `scripts/*.sh` file on
...
`SMARTPBX_HOST_FILES_SHA256=/opt/riviera/host-files.sha256` in the deploy
