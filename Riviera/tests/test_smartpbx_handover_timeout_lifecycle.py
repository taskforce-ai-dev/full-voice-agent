...
class Control:
    ...
    async def transfer_call(self,_): self.calls+=1; return type("R",(),{"acknowledged":self.ok})()
class Pipeline:
...
async def test_acknowledgement_does_not_complete_terminal_until_real_session_finish():
    ...
    session=RivieraSmartPBXSession(context,Transport(),pipeline=pipeline,post_call_processor=post,welcome_text="",llm_provider="openai",model="m")
    ...
    assert json.loads(await c.attempt("help"))["status"] == "transfer_requested"
