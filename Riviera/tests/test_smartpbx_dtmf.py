...
so the digit a guest presses while Riya is still speaking the "key it in" prompt
...
async def run_call(messages, *, registry=None, factory=None):
    ...
    socket = FakeWebSocket(messages, token="test-token", header="X-Riviera-SmartPBX-Token")
...
async def test_concurrent_calls_never_receive_each_others_digits():
    ...
    first_socket = FakeWebSocket(
        ...
        token="test-token", header="X-Riviera-SmartPBX-Token",
    )
...
async def test_session_teardown_resolves_a_collection_started_during_the_prompt(monkeypatch):
    ...
    smartpbx = RivieraSmartPBXSession(
