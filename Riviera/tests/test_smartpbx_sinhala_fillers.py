...
def test_si_stt_phrase_list_contains_room_name_vocabulary():
    ...
    """
    for room in server.ROOM_TYPES_BY_PROPERTY[server.PROPERTY_RIVIERA]:
        assert room in server.SI_STT_PHRASE_LIST
    for word in (
        "Chalet", "Cabana", "Cottage", "Lagoon", "Garden", "Steel",
        "Wooden", "Double", "Triple", "Basic", "Single", "Family",
    ):
...
def test_sinhala_prompt_contains_room_name_mapping_hint():
    ...
    assert "Cabana" in prompt
    assert "Chalet" in prompt
    assert "Cottage" in prompt
    assert "Lagoon View Steel Cabana" in prompt
    # The hint maps Sinhala transliterations back to the official room words.
    assert "කබානා = Cabana" in prompt
    assert "කොටේජ් = Cottage" in prompt
