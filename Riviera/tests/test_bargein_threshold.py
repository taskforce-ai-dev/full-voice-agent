...
While Riya is speaking, ANY STT result used to clear her queued audio, so a
...
def test_should_barge_in_debounces_the_start_of_speech(monkeypatch):
    ...
    # Riya just started speaking: a transcript now is most likely her own echo.
...
async def test_interim_blip_while_speaking_does_not_barge_in(monkeypatch):
    ...
    assert barged == [], "a short blip must not clear Riya's audio"
