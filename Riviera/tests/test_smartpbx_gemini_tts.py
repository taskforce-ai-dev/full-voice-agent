...
async def test_real_selected_profiles_keep_tts_routing_owned_by_lang(monkeypatch):
    ...
    async def elevenlabs(text, **_kwargs):
        assert server.load_riviera_english_voice_profile() is canonical
    ...
    monkeypatch.setattr(
        server, "load_riviera_english_voice_profile",
    ...
    monkeypatch.setattr(server, "load_riviera_english_voice_profile", lambda: canonical)
...
async def test_smartpbx_english_still_routes_to_elevenlabs(monkeypatch):
    ...
    elevenlabs.assert_awaited_once_with(
        "Hello from Riya.", sentence=None, turn_generation=-1
    )
...
def test_smartpbx_status_json_exposes_sinhala_tts_degraded_flag(monkeypatch):
    ...
    request = SimpleNamespace(headers={"X-Riviera-SmartPBX-Token": "status-token"})
