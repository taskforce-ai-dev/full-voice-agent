...
def test_profile_selects_exact_semantics_and_redacts_voice_id():
    profile = load_riviera_english_voice_profile(
        {RIVIERA_EN_ELEVENLABS_VOICE_ID: "unit-test-canonical-voice"}
    )
    ...
    assert RIVIERA_EN_ELEVENLABS_VOICE_ID in repr(profile)
...
def test_profile_fails_closed_when_protected_value_is_absent_or_blank(value):
    ...
    if value is not None:
        environment[RIVIERA_EN_ELEVENLABS_VOICE_ID] = value

    with pytest.raises(ValueError, match=RIVIERA_EN_ELEVENLABS_VOICE_ID):
        load_riviera_english_voice_profile(environment)
...
def test_ci_runs_a_non_deploying_riviera_image_build_and_import_gate():
    ci_workflow = (REPOSITORY_ROOT / ".github" / "workflows" / "ci.yml").read_text(
        encoding="utf-8"
    )

    # The CI matrix keys on the agent FOLDER (Riviera), never the persona.
    assert '- "Riviera"' in ci_workflow
    assert "if: matrix.agent == 'Riviera'" in ci_workflow
    assert "working-directory: Riviera" in ci_workflow
