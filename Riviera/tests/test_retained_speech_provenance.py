...
`"Riviera"`, so a new role added without touching it would attribute the guest's
...
def test_unconfirmed_role_is_not_rendered_as_a_riviera_line():
    """The trap: `_format_transcript` maps every unknown role to "Riviera"."""
    ...
    assert "Riya: half heard fragment" not in rendered, (
...
async def test_n8n_transcript_payload_marks_unconfirmed_fragments(monkeypatch):
    ...
    assert "Riya: zero seven seven one two" not in transcript
...
def test_handoff_transcript_does_not_attribute_unconfirmed_speech_to_riviera():
    ...
    assert "Riya: half heard fragment" not in rendered, (
        "`who = 'Guest' if role == 'user' else 'Riya'` is a binary test — a new "
