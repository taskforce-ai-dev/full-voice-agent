"""Contract tests for Riya SmartPBX admission and lifecycle boundaries."""
...
class FakeWebSocket:
    def __init__(self, messages=(), token="test-token", header="X-Riviera-SmartPBX-Token"):
...
async def run(messages, *, configuration=None, registry=None, token="test-token", header="X-Riviera-SmartPBX-Token"):
...
def test_settings_default_to_the_riviera_token_header_and_documented_bounds():
    configuration = settings()

    assert configuration.auth_header_name == "X-Riviera-SmartPBX-Token"
...
async def test_gateway_routes_dtmf_to_the_session_and_still_observes(caplog):
    ...
    registry = SmartPBXSessionRegistry(4)
    socket = FakeWebSocket([START, dtmf, {"event": "stop"}], token="test-token", header="X-Riviera-SmartPBX-Token")
