...
Asserts the TwiML branch that routes an unanswered transfer back into Riya in
failsafe mode, the carry-over state that survives the relay restart, and the
recovery system prompt Riya runs on.
"""
...
def test_unanswered_call_reenters_riviera_in_failsafe_mode(client, status):
    server._remember_handoff("CA_TEST_1", reason="wants a discount", caller_phone="+94771234567")
...
def test_prompt_offers_the_caller_id_when_we_have_it():
    ...
    assert "Riya: Hello Chanya, how can I help?" in prompt
...
def test_prompt_forbids_an_extra_confirmation_round_trip():
    """Regression: a read-back-the-number rule made Riya re-confirm a number the
...
def test_prompt_forbids_narrating_internal_state():
    """Regression: on a live call Riya opened with "I don't have your name from
