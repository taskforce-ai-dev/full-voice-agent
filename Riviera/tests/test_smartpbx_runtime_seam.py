...
def _session(pipeline, *, diagnostic_sink=None) -> RivieraSmartPBXSession:
    async def post(**_):
        raise AssertionError("empty transcript must not post")

    return RivieraSmartPBXSession(
...
class FailingSocket:
    ...
    def __init__(self, ok_sends: int = 0) -> None:
        ...
        self.sent = 0
        self.headers = {"X-Riviera-SmartPBX-Token": "token"}
        self.accepted = False
