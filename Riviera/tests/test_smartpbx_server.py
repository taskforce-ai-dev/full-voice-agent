"""Riya SmartPBX media-session and service-mode contract tests."""
...
async def test_smartpbx_english_tts_uses_profile_without_general_voice(monkeypatch):
    ...
    profile = load_riviera_english_voice_profile({"RIVIERA_EN_ELEVENLABS_VOICE_ID": "unit-test-canonical-voice"})
    monkeypatch.setattr(server, "ELEVENLABS_API_KEY", "unit-test-api-key")
    monkeypatch.setattr(server, "ELEVENLABS_VOICE_ID", "")
    monkeypatch.setattr(server, "load_riviera_english_voice_profile", lambda: profile)
    monkeypatch.setattr(server.httpx, "AsyncClient", lambda: client)
    pipeline = server.MediaStreamSession(websocket=None, lang="en", media_transport=FakeTransport())
    await pipeline._tts_elevenlabs("Hello from Riya.")
    ...
    assert request["json"] == {"text": "Hello from Riya.", "model_id": "eleven_flash_v2_5", "voice_settings": {"stability": 0.5, "similarity_boost": 0.75, "style": 0.0, "use_speaker_boost": True}}
...
async def test_smartpbx_english_tts_fails_closed_when_profile_is_unavailable(monkeypatch):
    ...
    monkeypatch.setattr(server, "load_riviera_english_voice_profile", lambda: (_ for _ in ()).throw(ValueError("RIVIERA_EN_ELEVENLABS_VOICE_ID must be configured")))
    monkeypatch.setattr(server.httpx, "AsyncClient", lambda: client)
    pipeline = server.MediaStreamSession(websocket=None, lang="en", media_transport=FakeTransport())
    await pipeline._tts_elevenlabs("Hello from Riya.")
...
async def test_smartpbx_english_tts_profile_failure_emits_finite_diagnostic(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        server,
        "load_riviera_english_voice_profile",
...
def make_session(*, post_call_processor, pipeline=None, stt=None, transport=None):
    ...
    session = RivieraSmartPBXSession(
        ...
        welcome_text="Welcome to Riviera Resort.",
        llm_provider="claude",
        model="test-model",
    )
...
@pytest.mark.asyncio
async def test_default_smartpbx_welcome_reuses_riviera_english_greeting():
    import server
...
@pytest.mark.parametrize(
    ...
    [
        b"",
        b"\xff" * 2400 + b"\x7f",
        b"\x00" * 2400 + b"\x7f" * 160,
        b"\xff" * 2560,
        b"\xff" * 2400 + b"\x7f" * (513 * 160 - 2400),
    ],
)
def test_language_menu_loader_rejects_invalid_wire_assets(monkeypatch, tmp_path, asset):
    asset_path = tmp_path / "menu.ulaw"
    asset_path.write_bytes(asset)
    monkeypatch.setattr(smartpbx_session, "_LANGUAGE_MENU_AUDIO_PATH", asset_path)
    smartpbx_session._load_smartpbx_language_menu_audio.cache_clear()
    try:
        with pytest.raises(RuntimeError, match="language menu asset is invalid"):
            smartpbx_session._load_smartpbx_language_menu_audio()
    finally:
        smartpbx_session._load_smartpbx_language_menu_audio.cache_clear()
...
async def test_dialog_finish_cancels_pending_initial_filler_before_other_teardown_awaits():
    """Item 2 (PR-266 review round 2): RivieraSmartPBXSession._finish_once must
...
@pytest.mark.asyncio
async def test_dialog_hangup_finishes_once_and_schedules_riviera_post_call():
    post_call_calls = []
...
async def test_smartpbx_english_pipeline_uses_existing_elevenlabs_tts():
    ...
    await pipeline._speak("Hello from Riya.")

    assert spoken == ["Hello from Riya."]
...
@pytest.mark.asyncio
async def test_pilot_transcript_logging_emits_exact_riviera_tts_phrase(
    caplog, monkeypatch,
):
    import server

    riviera_phrase = "Let us try that number again."
    pipeline = server.MediaStreamSession(
        websocket=None, lang="en", media_transport=FakeTransport()
    )
    pipeline._smartpbx_transfer_context = object()
    ...
    with caplog.at_level(logging.INFO):
        await pipeline._speak(riviera_phrase)

    assert spoken == [riviera_phrase]
    assert (
        "smartpbx_pilot_transcript role=riviera "
        f"text={riviera_phrase!r}"
    ) in caplog.text
...
def test_smartpbx_mode_exposes_only_bounded_routes():
    ...
    status = routes["/smartpbx/status"].endpoint(
        _fake_request({"X-Riviera-SmartPBX-Token": "do-not-expose"})
    )
...
def test_smartpbx_status_reports_transfer_enabled_only_for_complete_allowlisted_configuration():
    ...
    assert routes["/smartpbx/status"].endpoint(
        _fake_request({"X-Riviera-SmartPBX-Token": "test-token"})
...
def test_unknown_service_mode_fails_closed():
    ...
    with pytest.raises(ValueError, match="invalid RIVIERA_SERVICE_MODE"):
...
def test_smartpbx_session_declares_optional_diagnostic_sink_and_installs_before_welcome():
    ...
    parameters = inspect.signature(RivieraSmartPBXSession).parameters
...
async def test_smartpbx_session_installs_default_noop_sink_on_pipeline_before_welcome():
    ...
    try:
        ...
        assert pipeline.spoken == ["Welcome to Riviera Resort."]
    finally:
        await session.finish(False)
...
async def test_default_diagnostic_sink_is_callable_before_welcome_speak():
    ...
    try:
        ...
        sink = getattr(pipeline, "_smartpbx_diagnostic_sink", None)
        assert callable(sink)
        assert pipeline.sinks_seen_at_speak == [sink]
    finally:
        await session.finish(False)
...
async def test_explicit_diagnostic_sink_reaches_pipeline_before_welcome_by_identity():
    ...
    session = RivieraSmartPBXSession(
        ...
        post_call_processor=process_post_call, welcome_text="Welcome to Riviera Resort.",
...
def test_smartpbx_status_requires_the_shared_token():
    ...
    allowed = status(_fake_request({"X-Riviera-SmartPBX-Token": "status-token"}))
    ...
    for headers in (
        ...
        {"X-Riviera-SmartPBX-Token": "wrong-token12"},
        {"X-Riviera-SmartPBX-Token": "status-toke"},
        {"X-Riviera-SmartPBX-Token": "status-tokenn"},
        ...
        {"X-Riviera-SmartPBX-Token": "caf\xe9-status-token"},
    ):
...
def test_smartpbx_status_honours_a_renamed_auth_header():
    ...
    with pytest.raises(HTTPException):
        status(_fake_request({"X-Riviera-SmartPBX-Token": "status-token"}))
...
@pytest.mark.asyncio
@pytest.mark.parametrize("provider", ["openai", "gemini", "claude"])
@pytest.mark.parametrize("termination", ["barge_in", "session_finish", "runner_cancel"])
async def test_later_transfer_early_exit_cancels_deferred_model_tts(
    monkeypatch, provider, termination,
):
    """A later transfer batch may not strand a pre-transfer TTS task on exit."""
    import server
    ...
    held_tts: asyncio.Task | None = None

    async def tts(text, **_kwargs):
        nonlocal held_tts
        assert text == model_preamble.strip()
        held_tts = asyncio.current_task()
        tts_started.set()
        try:
            await asyncio.Event().wait()
        except asyncio.CancelledError:
    ...
    async def execute(name, _arguments):
        assert name == "check_availability", "transfer must not execute after ownership loss"
        check_started.set()
    ...
    try:
        ...
        if termination == "barge_in":
            await pipeline._handle_bargein()
            check_release.set()
            await asyncio.wait_for(turn, timeout=1)
        elif termination == "session_finish":
...
@pytest.mark.parametrize("provider", ["openai", "gemini", "claude"])
@pytest.mark.parametrize(
    ("capture_tool", "capture_result", "slot", "expected"),
    [
        (
        ...
        (
            "capture_spoken_name",
            {"status": "captured", "name": "Ada Lovelace"},
            "guest_name",
            "Ada Lovelace",
        ),
    ],
)
async def test_capture_then_transfer_keeps_capture_slot_without_late_completion(
    monkeypatch, provider, capture_tool, capture_result, slot, expected,
):
    """A quiet capture before transfer must commit once under the transfer fence."""
    import server
    import tools
    ...
    client = direct_tool_client(provider, [
        direct_tool_batch_round(
            provider,
            [
                (capture_tool, {"spoken": "raw capture"}),
                ("transfer_to_human", {"reason": "human please"}),
            ],
        ),
    ])
    ...
    async def execute(name, arguments):
        if name == capture_tool:
            return json.dumps(capture_result)
        assert name == "transfer_to_human"
        return await real_execute(name, arguments)

    def emit(_event, **fields):
        if fields.get("stage") == "late_tool_completion":
            telemetry.append("late_tool_completion")
    ...
    monkeypatch.setattr(pipeline, "_tts_elevenlabs", tts)
    monkeypatch.setattr(server, "execute_tool", execute)
    monkeypatch.setattr(server, "_emit_smartpbx_turn_telemetry", emit)
    monkeypatch.setattr(tools, "_await_turn_delivery", delivery_barrier)

    await asyncio.wait_for(pipeline._process_utterance("capture then human"), timeout=1)

    assert pipeline._booking_slots[slot] == expected
    assert timeline == ["canonical", "delivery_barrier", "mcp"]
    assert pipeline._media_transport.clears == 1
    assert telemetry == []
