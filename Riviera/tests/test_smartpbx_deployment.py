...
def test_riviera_smartpbx_is_loopback_only_and_uses_its_own_port():
    compose = yaml.safe_load(read_text("docker-compose.yml"))
    service = compose["services"]["riviera-smartpbx"]

    assert service["profiles"] == ["smartpbx"]
    assert service["image"] == "ghcr.io/taskforce-ai-dev/riviera:${SMARTPBX_IMAGE_TAG:-disabled}"
    # Membership alone (`in`) is satisfied by a second, additional binding
    # such as `0.0.0.0:8061:8000` -- pin there is exactly one published
    # port so a public bind can never sneak in alongside the loopback one.
    assert len(service["ports"]) == 1
    assert "127.0.0.1:8061:8000" in service["ports"]
    assert service["environment"]["RIVIERA_SERVICE_MODE"] == "smartpbx"
    assert service["environment"]["ENABLE_SMARTPBX_WSS"] == "true"
    assert service["environment"]["SMARTPBX_AUTH_HEADER_NAME"] == "X-Riviera-SmartPBX-Token"
    assert service["environment"]["SMARTPBX_MAX_CALLS"] == "4"


def test_every_riviera_smartpbx_env_var_has_a_compose_default():
    """P1-4: a key missing from .env.smartpbx must never arrive as "" -- every
    ``${VAR}`` in the riviera-smartpbx environment block must carry a ``:-``
    ...
    environment = compose["services"]["riviera-smartpbx"]["environment"]
    ...
    assert bare == [], f"these riviera-smartpbx env entries have a ${{VAR}} with no default: {bare}"
    ...
    image = compose["services"]["riviera-smartpbx"]["image"]
...
def test_nginx_exposes_only_the_bounded_smartpbx_surface_with_tls():
    ...
    assert "server_name smartpbx-riviera.taskforceai.tech;" in nginx
    ...
    assert "access_log /var/log/nginx/smartpbx-riviera-access.log" in nginx
    ...
    assert "access_log /var/log/nginx/smartpbx-riviera-access.log riviera_smartpbx_min;" in nginx
    assert re.search(
        r"log_format\s+riviera_smartpbx_min\s+'\$request_method \$uri \$status \$http_upgrade \$request_time';",
        nginx,
    )
    assert "limit_req_zone $binary_remote_addr zone=riviera_smartpbx_req:10m rate=30r/m;" in nginx
    media = nginx.split("location = /ws/v1/smartpbx/media", 1)[1].split("location =", 1)[0]
    assert "limit_req zone=riviera_smartpbx_req burst=10 nodelay;" in media
    ...
    assert "limit_req_zone $binary_remote_addr zone=riviera_smartpbx_http:10m rate=60r/m;" in nginx
    ...
    assert "limit_req zone=riviera_smartpbx_http burst=20 nodelay;" in health
    assert "limit_req zone=riviera_smartpbx_http burst=20 nodelay;" in status_block
    ...
    assert "proxy_pass http://127.0.0.1:8061;" in nginx
...
def test_runbook_keeps_credentials_server_side_and_documents_dialog_cutover():
    ...
    for required in (
        "wss://smartpbx-riviera.taskforceai.tech/ws/v1/smartpbx/media",
...
def test_smartpbx_reviewed_isolation_and_operations_contract():
    ...
    nginx = read_text("nginx-smartpbx.conf")
    for required in ("limit_conn riviera_smartpbx_per_ip 8;", "client_max_body_size 64k;", "client_header_timeout 10s;", "client_body_timeout 10s;", "proxy_connect_timeout 5s;", "proxy_buffering off;", "proxy_request_buffering off;"):
        assert required in nginx
    assert nginx.count("proxy_read_timeout 15s;") == 2
    assert nginx.count("proxy_send_timeout 15s;") == 2
    assert "limit_conn_zone $binary_remote_addr zone=riviera_smartpbx_per_ip:10m;" in nginx

    runbook = read_text("SMARTPBX_RUNBOOK.md")
    for required in ("cd /opt/riviera", "openssl rand -hex 32", "chmod 600 /opt/riviera/.env.smartpbx", "docker compose --env-file .env.smartpbx --profile smartpbx config > /dev/null", "REVIEWED_CI_SHORT_SHA", "REVIEWED_FULL_COMMIT_SHA", "Only the dedicated WSS token is pasted into the Dialog dashboard", "Riya accepts", "4 accepted + 5th rejected", "source-IP allowlist", "drain active calls", "Flico untouched"):
        assert required in runbook

    server = read_text("server.py")
    assert 'if RIVIERA_SERVICE_MODE != "smartpbx":' in server
...
def test_smartpbx_sinhala_phrase_cache_volume_and_env_have_compose_defaults():
    ...
    riviera_ignore = read_text(".gitignore")
    assert "smartpbx_phrase_cache/" in root_ignore
    assert "smartpbx_phrase_cache/" in riviera_ignore
...
def test_acme_bootstrap_precedes_loopback_health_and_final_tls_proxy():
    ...
    positions = [
        runbook.find("getent ahostsv4 smartpbx-riviera.taskforceai.tech"),
        ...
        runbook.find("up -d --force-recreate --pull never riviera-smartpbx"),
        runbook.find("http://127.0.0.1:8061/health"),
        runbook.find("sudo install -m 0644 nginx-smartpbx.conf"),
        runbook.rfind("sudo nginx -t"),
        runbook.rfind("sudo systemctl reload nginx"),
        runbook.rfind("https://smartpbx-riviera.taskforceai.tech/health"),
    ]
...
def test_tls_and_mcp_recreates_fail_fast_and_wait_for_bounded_loopback_readiness():
    ...
    for block in (tls, enable, revoke):
        ...
        assert "curl --silent --show-error --fail --connect-timeout 2 --max-time 5 http://127.0.0.1:8061/health" in block
        assert "curl --silent --show-error --fail --connect-timeout 2 --max-time 5 http://127.0.0.1:8061/smartpbx/status" in block
...
def test_yanolja_credentials_are_blank_and_smartpbx_chroma_state_is_ignored():
    ...
    pms_runbook = read_text("ops/riviera-pms/RUNBOOK.md")
    root_ignore = (PROJECT_ROOT.parent / ".gitignore").read_text(encoding="utf-8")
    riviera_ignore = read_text(".gitignore")
    ...
    assert "chroma_db_smartpbx/" in riviera_ignore


def test_canonical_voice_configuration_covers_both_riviera_services_and_stays_disabled():
    example = read_text(".env.example")
    compose = yaml.safe_load(read_text("docker-compose.yml"))
    runbook = read_text("SMARTPBX_RUNBOOK.md")
    legacy = compose["services"]["riviera"]
    smartpbx = compose["services"]["riviera-smartpbx"]
    environment = smartpbx["environment"]

    assert re.search(r"^RIVIERA_EN_ELEVENLABS_VOICE_ID=$", example, re.MULTILINE)
    assert re.search(r"^RIVIERA_EN_ELEVENLABS_VOICE_ID=.+$", example, re.MULTILINE) is None
    assert legacy["env_file"] == [".env"]
    # P1-4: these still resolve to their fail-closed/disabled state on a blank
    # value, so the compose default is empty (RIVIERA_EN_ELEVENLABS_VOICE_ID,
    ...
    assert environment["RIVIERA_EN_ELEVENLABS_VOICE_ID"] == "${RIVIERA_EN_ELEVENLABS_VOICE_ID:-}"
    ...
    for required in (
        ...
        "sudo chown root:root /opt/riviera/.env /opt/riviera/.env.smartpbx",
        "sudo chmod 600 /opt/riviera/.env /opt/riviera/.env.smartpbx",
        "sudo /opt/riviera/scripts/validate_english_voice_env.sh /opt/riviera/.env /opt/riviera/.env.smartpbx",
...
def test_cutover_gates_require_fixed_private_protocol_diagnostics_and_preserve_operations():
    ...
    for required in ("REVIEWED_CI_SHORT_SHA", "REVIEWED_FULL_COMMIT_SHA", "--pull never", "riviera-smartpbx"):
        assert required in runbook
    ...
    stop = rollback.find("stop riviera-smartpbx")
    assert withdraw >= 0
...
def test_claude_output_budget_knob_is_a_blank_safe_passthrough_everywhere():
    ...
    the shared one: allowlisted in the riviera-smartpbx service, present but
...
def test_claude_md_warns_that_deploy_sh_and_vps_builds_are_not_for_smartpbx():
    ...
    assert "riviera-smartpbx" in commands_section
    ...
    assert "riviera-smartpbx" in operational_section
...
def test_pr_impact_keeps_riviera_out_of_generic_deploy_and_gives_its_real_rollback_path():
    workflow = (
    ...
    assert (
        "Do not call generic `deploy.yml` with `agent=riviera`; that route rejects Riviera."
        in workflow
    )
    assert "# registry agents (all seven active ones)" not in workflow


BUILD_RIVIERA_IMAGE_WORKFLOW = PROJECT_ROOT.parent / ".github/workflows/build-riviera-image.yml"
DEPLOY_WORKFLOW = PROJECT_ROOT.parent / ".github/workflows/deploy.yml"


def read_build_riviera_image_workflow():
    assert BUILD_RIVIERA_IMAGE_WORKFLOW.is_file(), (
        "missing build-only Riviera image publisher workflow"
    )
    text = BUILD_RIVIERA_IMAGE_WORKFLOW.read_text(encoding="utf-8")
    document = yaml.load(text, Loader=yaml.BaseLoader)
    assert isinstance(document, dict)
    return document, text


def build_riviera_image_job():
    document, text = read_build_riviera_image_workflow()
...
RIVIERA_IMAGE_PROBE_WORKFLOW = PROJECT_ROOT.parent / ".github/workflows/probe-riviera-image.yml"
# The read-only probe design was written for Kavya; Riviera's probe workflow is
# a byte-for-byte clone of Kavya's with the image/event names swapped, so the
# same design document (and its dispatcher command shape) governs it.
RIVIERA_PROBE_SPEC = (
    PROJECT_ROOT.parent / "docs" / "superpowers" / "specs"
    / "2026-08-08-kavya-ghcr-read-only-probe-design.md"
)


def read_riviera_image_probe_workflow():
    assert RIVIERA_IMAGE_PROBE_WORKFLOW.is_file(), "missing read-only Riviera image probe workflow"
    text = RIVIERA_IMAGE_PROBE_WORKFLOW.read_text(encoding="utf-8")
    document = yaml.load(text, Loader=yaml.BaseLoader)
    assert isinstance(document, dict)
    return document, text


def riviera_image_probe_job():
    document, text = read_riviera_image_probe_workflow()
    jobs = document.get("jobs", {})
...
@pytest.mark.parametrize(
    "workflow", [RIVIERA_IMAGE_PROBE_WORKFLOW, BUILD_RIVIERA_IMAGE_WORKFLOW], ids=["probe", "publisher"]
)
def test_riviera_image_probe_and_publisher_pin_actions_to_full_commit_shas(workflow):
    ...
    for reference in references:
        ...
        assert owner_repo in RIVIERA_WORKFLOW_ACTION_PINS, owner_repo
        expected_sha, expected_version = RIVIERA_WORKFLOW_ACTION_PINS[owner_repo]
        assert sha == expected_sha, reference
        assert f"uses: {reference} # {expected_version}\n" in text, reference


def test_riviera_image_probe_and_publisher_fix_the_runner_label_and_job_timeout():
    _probe_document, probe_job, _probe_steps, _probe_text = riviera_image_probe_job()
    _build_document, build_job, _build_steps, _build_text = build_riviera_image_job()
...
PROBE_WORKFLOW_REF = f"{PROBE_REPOSITORY}/.github/workflows/probe-riviera-image.yml@refs/heads/main"
...
def test_riviera_image_probe_workflow_has_read_only_dispatch_trust_contract():
    document, job, steps, _text = riviera_image_probe_job()

    assert document["name"] == "Probe Riviera image (read-only)"
    assert document["on"] == {"repository_dispatch": {"types": ["riviera_image_read_only_probe"]}}
    assert document["concurrency"] == {"group": "riviera-image-read-only-probe", "cancel-in-progress": "false"}
...
def test_riviera_image_probe_binds_execution_to_protected_default_branch():
    document, _job, steps, text = riviera_image_probe_job()
    ...
    for check in (
        ...
        "[[ \"$PROBE_EVENT_ACTION\" == 'riviera_image_read_only_probe' ]] || fail",
        ...
        '[[ "$PROBE_WORKFLOW_REF" == "$repository/.github/workflows/probe-riviera-image.yml@refs/heads/main" ]] || fail',
...
def test_riviera_image_probe_dispatcher_is_terminal_and_sends_no_ref_selector():
    spec = RIVIERA_PROBE_SPEC.read_text(encoding="utf-8")
    ...
    assert "-f event_type=kavya_image_read_only_probe" in dispatcher  # riviera_image_read_only_probe for Riviera
...
def test_riviera_image_probe_validation_precedes_all_tooling_and_has_no_source_checkout():
    document, _job, steps, text = riviera_image_probe_job()
    validation = probe_workflow_step(steps, PROBE_VALIDATION_STEP)
    checkout = probe_workflow_step(steps, "Checkout trusted probe tooling")
    ...
    for command in ("docker ", "check-riviera-image-tag.sh", "curl ", "wget "):
...
def test_riviera_image_probe_requires_exact_states_provenance_and_internal_canary():
    document, _job, steps, text = riviera_image_probe_job()
    ...
    assert RIVIERA_IMAGE_TAG_PROBE.is_file()
    assert ".probe-tools/.github/scripts/check-riviera-image-tag.sh" in existing["run"]
    assert "probe_code" in existing["run"]
    assert '[[ "$probe_code" -eq 10 ]]' in existing["run"]
    assert 'image="ghcr.io/taskforce-ai-dev/riviera"' in existing["run"]
...
def test_riviera_image_probe_has_no_write_deploy_or_sensitive_output_surface():
    _document, job, _steps, text = riviera_image_probe_job()
    runs = "\n".join(workflow_run_strings(riviera_image_probe_job()[0]))
...
def probe_event_payload(
    ...
    return json.dumps(
        {
            "action": "riviera_image_read_only_probe",
            "branch": branch,
...
def run_probe_workflow_step(tmp_path, name, output_files=None, **values):
    document, _job, steps, _text = riviera_image_probe_job()
    ...
    (scripts / "check-riviera-image-tag.sh").write_text(textwrap.dedent("""\
        #!/usr/bin/env bash
        set -Eeuo pipefail
        printf 'probe %s\\n' "$1" >> "$FAKE_LOG"
        if [[ "$1" == *":probe-"* ]]; then
    ...
    env = os.environ | {
        ...
        "PROBE_EVENT_ACTION": "riviera_image_read_only_probe",
...
def test_riviera_image_probe_validation_binds_identity_before_tools(tmp_path, tag, revision, code):
...
PROBE_UNTRUSTED_CONTEXTS = [
    ...
    {"PROBE_WORKFLOW_REF": f"{PROBE_REPOSITORY}/.github/workflows/probe-riviera-image.yml@refs/heads/rakesh"},
...
def test_riviera_image_probe_rejects_untrusted_dispatch_context(tmp_path, overrides):
...
def test_riviera_image_probe_payload_is_exactly_two_validated_string_keys(tmp_path, payload):
    result, summary, log = run_probe_workflow_step(
...
def test_riviera_image_probe_ignores_the_webhook_branch_field(tmp_path, branch):
...
def test_riviera_image_probe_accepts_only_exact_probe_states(tmp_path, step, overrides, code):
    ...
    if step == "Probe generated absent canary":
        assert log == "probe ghcr.io/taskforce-ai-dev/riviera:probe-123-456-1\n"


PROBE_DIGEST_INSPECT_LOG = (
    f"docker buildx imagetools inspect ghcr.io/taskforce-ai-dev/riviera:{PROBE_EXISTING_TAG}"
    " --format {{ .Manifest.Digest }}"
)
PROBE_PULL_LOG = f"docker pull ghcr.io/taskforce-ai-dev/riviera@{PROBE_CANARY_DIGEST}"
PROBE_IMAGE_INSPECT_LOG = (
    f"docker image inspect ghcr.io/taskforce-ai-dev/riviera@{PROBE_CANARY_DIGEST}"
...
def test_riviera_image_probe_provenance_and_metadata_suppress_sentinels(tmp_path, overrides, code, expected_log):
    result, summary, log = run_probe_workflow_step(
...
def test_riviera_image_probe_canary_identifier_table_fails_before_fake_tools(tmp_path, repository_id, run_id, attempt):
...
def test_riviera_image_probe_metadata_table_is_safe(tmp_path, overrides, code):
...
def test_riviera_image_probe_rejects_nul_bearing_contract_bytes(tmp_path, step, output_files):
...
def test_riviera_image_probe_run_scripts_parse_and_summary_is_allowlisted(tmp_path):
    document, _job, steps, _text = riviera_image_probe_job()
...
def test_build_riviera_image_publisher_is_dispatch_only_and_least_privilege():
    document, job, steps, text = build_riviera_image_job()

    assert document["name"] == "Build Riviera image (no deploy)"
...
PUBLISHER_SHA = "c" * 40
PUBLISHER_WORKFLOW_REF = f"{PROBE_REPOSITORY}/.github/workflows/build-riviera-image.yml@refs/heads/main"
...
# The real index digest build-push-action reported for riviera:db7d0d1.
PUBLISHER_BUILT_DIGEST = "sha256:3d7f73715d934dbb30e7ac09f240162830ee9d8038b04a73554e9539247d88a7"


def test_build_riviera_image_publisher_binds_execution_to_protected_default_branch():
    document, _job, steps, _text = build_riviera_image_job()
    ...
    for check in (
        ...
        '[[ "$PUBLISHER_WORKFLOW_REF" == "$repository/.github/workflows/build-riviera-image.yml@refs/heads/main" ]] || fail',
    ):
        assert check in trust["run"], check
    assert "gate_sha=%s" in trust["run"]


def test_build_riviera_image_publisher_requires_a_green_probe_before_any_credentialed_step():
    document, _job, steps, text = build_riviera_image_job()
    ...
    assert 'actions/workflows/probe-riviera-image.yml/runs?' in run
    ...
    for check in (
        ...
        '.path == ".github/workflows/probe-riviera-image.yml"',
        '.head_branch == "main"',
        '.status == "completed"',
        '.conclusion == "success"',
...
def run_publisher_workflow_step(tmp_path, name, gh_stdout=None, **values):
    _document, _job, steps, _text = build_riviera_image_job()
    ...
    env = os.environ | {
        ...
        "MODE": "absent", "TAG": "ghcr.io/taskforce-ai-dev/riviera:db7d0d1",
...
PUBLISHER_UNTRUSTED_CONTEXTS = [
    ...
    {"PUBLISHER_WORKFLOW_REF": f"{PROBE_REPOSITORY}/.github/workflows/build-riviera-image.yml@refs/heads/rakesh"},
...
def test_build_riviera_image_publisher_trust_step_rejects_untrusted_context(tmp_path, overrides):
...
def test_build_riviera_image_publisher_trust_step_publishes_the_verified_sha(tmp_path):
...
def test_build_riviera_image_publisher_probe_gate_fails_closed(tmp_path, gh_stdout, gh_code, code):
...
def test_build_riviera_image_publisher_probe_gate_queries_the_bound_commit(tmp_path):
    result, outputs, log = run_publisher_workflow_step(tmp_path, PUBLISHER_GATE_STEP)

    assert result.returncode == 0
    assert log.splitlines() == [
        "gh api "
        f"repos/{PROBE_REPOSITORY}/actions/workflows/probe-riviera-image.yml/runs"
...
def test_build_riviera_image_publisher_summary_records_the_probe_gate_evidence():
    _document, _job, steps, _text = build_riviera_image_job()
...
def test_build_riviera_image_publisher_validates_the_checked_out_review_before_registry_access():
    _document, _job, steps, _text = build_riviera_image_job()
...
def test_build_riviera_image_publisher_uses_one_immutable_checked_out_tag_and_verifies_digest():
    _document, job, steps, text = build_riviera_image_job()
    ...
    assert "image=\"ghcr.io/taskforce-ai-dev/riviera\"" in identity["run"]
    assert "tag=\"${image}:${actual_sha::7}\"" in identity["run"]
    assert build["uses"].startswith("docker/build-push-action@")
    assert build["with"]["context"] == "source/Riviera"
    assert build["with"]["file"] == "source/Riviera/Dockerfile"
...
def test_build_riviera_image_publisher_summary_is_limited_to_safe_build_identity():
    _document, _job, steps, _text = build_riviera_image_job()
...
RIVIERA_IMAGE_TAG_PROBE = PROJECT_ROOT.parent / ".github/scripts/check-riviera-image-tag.sh"
RIVIERA_IMAGE_TARGET = "ghcr.io/taskforce-ai-dev/riviera:deadbee"
...
def run_riviera_image_tag_probe(
    tmp_path,
    docker_exit,
    docker_output,
    target=RIVIERA_IMAGE_TARGET,
    output_bytes=None,
    binary=False,
    argument=None,
):
    assert RIVIERA_IMAGE_TAG_PROBE.is_file(), "missing executable Riviera image tag probe"
    assert os.access(RIVIERA_IMAGE_TAG_PROBE, os.X_OK)
    fake_bin = tmp_path / "bin"
    ...
    return subprocess.run(
        [str(RIVIERA_IMAGE_TAG_PROBE), target if argument is None else argument],
        env=environment,
        text=not binary,
        capture_output=True,
        check=False,
...
CANARY_SHAPED_TARGET = "ghcr.io/taskforce-ai-dev/riviera:probe-1050123-17284593012-1"
PUBLISHER_SHAPED_TARGET = "ghcr.io/taskforce-ai-dev/riviera:a5012bc"


def test_build_riviera_image_publisher_uses_static_concurrency_and_env_only_input_flow():
    document, _job, steps, _text = build_riviera_image_job()
    ...
    assert concurrency["group"] == "riviera-image-publisher"
...
def test_build_riviera_image_publisher_probes_registry_before_build_with_an_executable_script():
    _document, _job, steps, text = build_riviera_image_job()
    ...
    assert RIVIERA_IMAGE_TAG_PROBE.is_file()
    probe_script = RIVIERA_IMAGE_TAG_PROBE.read_text(encoding="utf-8")
    assert "docker buildx imagetools inspect \"$TAG\"" in probe_script
    assert "docker manifest inspect" not in probe_script
    assert os.access(RIVIERA_IMAGE_TAG_PROBE, os.X_OK)
    assert "bash .publisher/.github/scripts/check-riviera-image-tag.sh \"$TAG\"" in probe["run"]
...
@pytest.mark.parametrize(
    ...
    [
        (
            0,
            "Name: ghcr.io/taskforce-ai-dev/riviera:deadbee\nMediaType: application/vnd.oci.image.index.v1+json\nManifests: linux/amd64, linux/arm64, provenance",
            10,
            "existing",
        ),
        (1, f"manifest unknown: {RIVIERA_IMAGE_TARGET}", 0, "absent"),
        (1, f"no such manifest: {RIVIERA_IMAGE_TARGET}", 0, "absent"),
        (1, f"failed to resolve source metadata for {RIVIERA_IMAGE_TARGET}: not found", 0, "absent"),
        ...
        (1, f"ERROR: {RIVIERA_IMAGE_TARGET}: not found", 0, "absent"),
        ...
        (1, f"denied: manifest unknown: {RIVIERA_IMAGE_TARGET}", 2, "probe_unrecognized"),
        (1, f"authentication required: no such manifest: {RIVIERA_IMAGE_TARGET}", 2, "probe_unrecognized"),
        (1, f"token expired: {RIVIERA_IMAGE_TARGET}: not found", 2, "probe_unrecognized"),
        (1, f"proxy returned 404 for {RIVIERA_IMAGE_TARGET}", 2, "probe_unrecognized"),
        (1, f"timeout: manifest unknown: {RIVIERA_IMAGE_TARGET}", 2, "probe_unrecognized"),
        (1, f"dial tcp: lookup registry: no such host: {RIVIERA_IMAGE_TARGET}: not found", 2, "probe_unrecognized"),
        (1, f"429 rate limit: no such manifest: {RIVIERA_IMAGE_TARGET}", 2, "probe_unrecognized"),
        (1, f"500 internal server error: {RIVIERA_IMAGE_TARGET}: not found", 2, "probe_unrecognized"),
        (1, "manifest unknown", 2, "probe_unrecognized"),
        (1, "no such manifest", 2, "probe_unrecognized"),
        (1, "not found", 2, "probe_unrecognized"),
        (1, "no such manifest: ghcr.io/taskforce-ai-dev/riviera:othertag", 2, "probe_unrecognized"),
        (1, "failed to resolve source metadata for ghcr.io/taskforce-ai-dev/riviera:othertag: not found", 2, "probe_unrecognized"),
        ...
        (1, f"ERROR: manifest unknown: {RIVIERA_IMAGE_TARGET}", 2, "probe_unrecognized"),
        (1, f"ERROR: {RIVIERA_IMAGE_TARGET}: not found extra", 2, "probe_unrecognized"),
        (1, f"prefix ERROR: {RIVIERA_IMAGE_TARGET}: not found", 2, "probe_unrecognized"),
        (1, "ERROR: ghcr.io/taskforce-ai-dev/riviera:othertag: not found", 2, "probe_unrecognized"),
    ],
)
def test_riviera_image_tag_probe_fails_closed_without_echoing_registry_errors(
    tmp_path, docker_exit, docker_output, expected_exit, expected_state
):
    result = run_riviera_image_tag_probe(tmp_path, docker_exit, docker_output)
...
def test_riviera_image_tag_probe_classifies_digit_bearing_tags_as_absent(
    ...
    result = run_riviera_image_tag_probe(
...
@pytest.mark.parametrize("target", [CANARY_SHAPED_TARGET, PUBLISHER_SHAPED_TARGET])
def test_riviera_image_tag_probe_still_fails_closed_on_real_server_errors(tmp_path, target):
    result = run_riviera_image_tag_probe(
...
def test_riviera_image_tag_probe_separates_unrecognised_messages_from_rejections(tmp_path):
    ...
    unrecognised = run_riviera_image_tag_probe(
        tmp_path / "a", 1, f"ERROR: {RIVIERA_IMAGE_TARGET}: unexpected status 418"
    )
    assert unrecognised.returncode == 2
    assert unrecognised.stdout == "image_tag_state=probe_unrecognized\n"

    nul_bearing = run_riviera_image_tag_probe(
        tmp_path / "b", 1, "", output_bytes=b"manifest unknown: " + RIVIERA_IMAGE_TARGET.encode() + b"\x00"
    )
    assert nul_bearing.returncode == 1
    assert nul_bearing.stdout == "image_tag_state=probe_failed\n"

    mixed_case = run_riviera_image_tag_probe(
        tmp_path / "c", 1, "manifest unknown", argument=RIVIERA_IMAGE_TARGET.upper()
    )
    assert mixed_case.returncode == 1
    assert mixed_case.stdout == "image_tag_state=probe_failed\n"

    no_argument = subprocess.run(
        [str(RIVIERA_IMAGE_TAG_PROBE)], text=True, capture_output=True, check=False
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (1, b"manifest unknown: " + RIVIERA_IMAGE_TARGET.encode("ascii") + b"\x00"),
        (1, b"\x00manifest unknown: " + RIVIERA_IMAGE_TARGET.encode("ascii")),
        (1, b"no such manifest: " + RIVIERA_IMAGE_TARGET.encode("ascii") + b"\x00\n"),
        # Docker success: a NUL must still fail closed rather than report existing.
        (0, b"Name: " + RIVIERA_IMAGE_TARGET.encode("ascii") + b"\x00\n"),
        (0, b"\x00"),
    ],
)
def test_riviera_image_tag_probe_rejects_nul_bearing_registry_captures(
...
@pytest.mark.parametrize(
    "argument",
    [
        "ghcr.io/taskforce-ai-dev/riviera:DEADBEE",
        "ghcr.io/taskforce-ai-dev/Riviera:deadbee",
        "GHCR.IO/taskforce-ai-dev/riviera:deadbee",
    ],
)
def test_riviera_image_tag_probe_rejects_mixed_case_arguments(tmp_path, argument):
    result = run_riviera_image_tag_probe(
...
def test_riviera_image_tag_probe_documents_its_nul_and_case_contract():
    script = RIVIERA_IMAGE_TAG_PROBE.read_text(encoding="utf-8")
...
def test_build_riviera_image_publisher_verifies_existing_tags_without_overwriting_them():
    _document, _job, steps, _text = build_riviera_image_job()
...
def test_build_riviera_image_publisher_uses_only_trusted_tooling_and_honest_writer_scope_note():
    document, _job, steps, text = build_riviera_image_job()
    ...
    assert ".publisher/.github/scripts/check-riviera-image-tag.sh" in probe["run"]
...
def test_deploy_workflow_rejects_riviera_for_every_mode_before_any_publisher_or_host_step():
...
def test_deploy_workflow_rsync_excludes_smartpbx_host_control_plane_for_riviera():
    text = DEPLOY_WORKFLOW.read_text(encoding="utf-8")
    document = yaml.load(text, Loader=yaml.BaseLoader)
    steps = document["jobs"]["deploy"]["steps"]
    ...
    sync_run = sync["run"]
    # Defence in depth: even if the reject guard above is ever loosened,
    ...
    # These extra excludes must be conditional on the SmartPBX agents (Kavya and
    # its Riviera clone) so a build-mode deploy of any other agent still ships
    # its own docker-compose.yml / scripts. Riviera must be named in that
    # guard: it carries the same host-side control plane as Kavya.
    guard_line = next(
        line for line in sync_run.splitlines()
        if line.lstrip().startswith("if ") and "$AGENT" in line
    )
    assert '"$AGENT" == "riviera"' in guard_line


# The generic deploy engine + push trigger + secret scan + dep audit are not
# gated by the reviewed-SHA image contract the Riviera-specific probe/publisher
...
# currently resolves to, with a `# vX` comment, matching build-riviera-image.yml.
...
def test_dep_audit_workflow_audits_rivieras_lock_file_and_blocks_only_riviera():
    workflow = (PROJECT_ROOT.parent / ".github/workflows/dep-audit.yml").read_text(encoding="utf-8")
    document = yaml.load(workflow, Loader=yaml.BaseLoader)

    # The advisory-vs-enforced split: every other agent stays advisory
    # (continue-on-error); Kavya and its Riviera clone are blocking. The
    # matrix is an expression: the blocking rows alone on pull_request (on a
    # trigger scoped to their requirements files), all nine agents on the
    # weekly/manual runs.
    ...
    for agent in (
        "BSL Agent", "Flico Agent", "HattonHills", "Kavya", "Kitchened",
        "Riviera", "SLIC Agent", "Sofia Agent", "WorldOfRefrigerators",
    ):
    ...
    run = step["run"]
    # Riviera (like Kavya) is audited from the exact lock file its Dockerfile
    # installs (Riviera/requirements-prod.lock.txt), not the loose
    # requirements-prod.txt ranges pip-audit would otherwise silently
    # re-resolve to the latest matching versions. The guard that selects the
    # lock-file branch must therefore name Riviera.
    assert 'req="$dir/requirements-prod.lock.txt"' in run
    assert 'pip-audit -r "$req" --no-deps' in run
    lock_guard = run.split('req="$dir/requirements-prod.lock.txt"', 1)[0].rstrip().splitlines()[-1]
    assert lock_guard.lstrip().startswith("if ")
    assert '"$dir" = "Riviera"' in lock_guard
    ...
    # A pull_request trigger exists, scoped to the blocking agents' own
    # requirements files only -- it must not reintroduce the per-PR cost for
    # the seven advisory agents. Riviera's row is blocking on pull_request,
    # so its requirements files must be in that path filter.
    on = document["on"]
    assert "pull_request" in on
    paths = on["pull_request"]["paths"]
    assert "Riviera/requirements*" in paths
    assert set(paths) <= {"Kavya/requirements*", "Riviera/requirements*"}
...
def test_smartpbx_image_deploy_helper_is_root_only_sourceable_and_uses_a_fixed_target():
    ...
    for required in (
        ...
        "cd \"$APP_DIR\"",
        "flock -n 9",
        "/var/lock/riviera-smartpbx-image-deploy.lock",
        "NEW_TAG=$1",
        "EXPECTED_SHA=$2",
        "EXPECTED_DIGEST=$3",
        "validate_inputs",
...
def test_smartpbx_image_deploy_helper_recreates_only_smartpbx_and_checks_json_readiness():
    ...
    mutation = "SMARTPBX_IMAGE_TAG=\"$TAG\" docker compose --env-file .env.smartpbx --profile smartpbx up -d --force-recreate --pull never riviera-smartpbx"
    assert mutation in script
    assert script.count("--force-recreate --pull never riviera-smartpbx") == 1
    assert "deadline=$((SECONDS + 90))" in script
    assert "http://127.0.0.1:8061/health" in script
    assert "http://127.0.0.1:8061/smartpbx/status" in script
    ...
    assert "docker inspect --format '{{.Image}}' riviera-smartpbx" in script
    ...
    for forbidden in (
        ...
        "riviera-smartpbx flico",
    ):
        assert forbidden not in script.lower()


def test_smartpbx_image_deploy_helper_isolation_baseline_defaults_to_riviera_only():
    """The baseline is a configurable neighbour loop whose default is Riviera's
    own Twilio container; no other agent's container is a hard dependency."""
    script = read_smartpbx_image_deploy_script()

    assert 'SMARTPBX_ISOLATION_NEIGHBOURS="${SMARTPBX_ISOLATION_NEIGHBOURS:-riviera-voice-agent}"' in script
    assert "capture_isolation_baseline()" in script
    assert "verify_isolation_baseline()" in script
    for function in ("capture_isolation_baseline", "verify_isolation_baseline"):
        body = script.split(f"{function}() {{", 1)[1].split("\n}", 1)[0]
        assert "for service in $SMARTPBX_ISOLATION_NEIGHBOURS; do" in body
    code = "\n".join(line for line in script.splitlines() if not line.lstrip().startswith("#"))
    for stale in ("FLICO_ID", "LEGACY_ID", "FLICO_HEALTH", "LEGACY_HEALTH",
                  "capture_service_snapshot flico-voice-agent",
                  "capture_service_snapshot riviera-voice-agent",
                  "kavya-voice-agent", "kavya-smartpbx"):
...
def test_smartpbx_image_deploy_helper_prunes_stale_images_only_after_disarm():
    ...
    assert "prune_stale_riviera_images" in script
    ...
    assert main_block.find("prune_stale_riviera_images") < main_block.find("verify_candidate_image")
    armed_window = main_block.split("arm_rollback", 1)[1].split("disarm_rollback", 1)[0]
    assert "prune_stale_riviera_images" not in armed_window
    assert main_block.rfind("prune_stale_riviera_images") > main_block.find("disarm_rollback")
    assert "prune_stale_riviera_images" not in script.split("rollback_once() {", 1)[1].split("\n}", 1)[0]
    prune_body = script.split("prune_stale_riviera_images() {", 1)[1].split("\nmain() {", 1)[0]
...
def test_smartpbx_image_deploy_helper_rolls_back_exact_local_baseline_without_sensitive_output():
    ...
    for forbidden in (
        ...
        "curl http://127.0.0.1:8061/smartpbx/status",
        "docker compose logs",
    ):
        assert forbidden not in script
...
def test_prune_stale_riviera_images_keeps_running_rollback_stable_and_latest(tmp_path):
    ...
    (fake_bin / "docker").write_text(
        "#!/usr/bin/env bash\n"
        'printf \'%s\\n\' "$*" >> "$DOCKER_CALL_LOG"\n'
        'if [[ $1 == inspect && $4 == riviera-smartpbx ]]; then printf \'%s\\n\' "ghcr.io/taskforce-ai-dev/riviera:abcdef0"; exit 0; fi\n'
        'if [[ $1 == inspect && $4 == riviera-voice-agent ]]; then printf \'%s\\n\' "ghcr.io/taskforce-ai-dev/riviera:1234567"; exit 0; fi\n'
        'if [[ $1 == images ]]; then\n'
        '  printf \'%s\\n\' \\\n'
        '    "ghcr.io/taskforce-ai-dev/riviera:abcdef0" \\\n'
        '    "ghcr.io/taskforce-ai-dev/riviera:1234567" \\\n'
        '    "ghcr.io/taskforce-ai-dev/riviera:rollback-local" \\\n'
        '    "ghcr.io/taskforce-ai-dev/riviera:latest" \\\n'
        '    "ghcr.io/taskforce-ai-dev/riviera:stable-20260901" \\\n'
        '    "ghcr.io/taskforce-ai-dev/riviera:0ddba11"\n'
        '  exit 0\n'
        'fi\n'
    ...
    source = f"source {SMARTPBX_IMAGE_DEPLOY_SCRIPT}; prune_stale_riviera_images"
    result = subprocess.run(["bash", "-c", source], env=env, text=True, capture_output=True, check=False)
    assert result.returncode == 0
    calls = log_file.read_text(encoding="utf-8")
    assert "rmi ghcr.io/taskforce-ai-dev/riviera:0ddba11" in calls
    for kept in ("abcdef0", "1234567", "rollback-local", "latest", "stable-20260901"):
        assert f"rmi ghcr.io/taskforce-ai-dev/riviera:{kept}" not in calls
...
class FakeDeployHost:
    ...
    image = "ghcr.io/taskforce-ai-dev/riviera"
    sha = "abcdef0" + "1" * 33
    digest = "sha256:" + "d" * 64
    ...
    def __init__(self, tmp_path, **state):
        ...
        # /var/log/riviera-smartpbx -- confine it to this scenario's tmp_path.
    ...
    def _docker(self):
        ...
        path.write_text(textwrap.dedent("""\
            ...
            def image_for(ref):
                ...
                if not state['mutated'] and os.getenv('CANDIDATE_DIGEST_BAD') == '1': digest='ghcr.io/taskforce-ai-dev/riviera@sha256:'+'9'*64
                if not state['mutated'] and os.getenv('CANDIDATE_REVISION_BAD') == '1': revision='9'*40
                if state['mutated'] and ref == state['current_id']:
                    if os.getenv('FORWARD_DIGEST_BAD') == '1': digest='ghcr.io/taskforce-ai-dev/riviera@sha256:'+'9'*64
            ...
            if args[:2] == ['inspect', '--format']:
                ...
                if name == 'riviera-voice-agent' and os.getenv('LEGACY_ABSENT') == '1': sys.exit(1)
                values={'riviera-smartpbx': {'{{.Image}}':state['current_id']}, 'flico-voice-agent': {'{{.Id}}':state['flico_id'],'{{.State.Health.Status}}':state['flico_health']}, 'riviera-voice-agent': {'{{.Id}}':state['legacy_id'],'{{.State.Health.Status}}':state['legacy_health']}}
            ...
            if args[:2] == ['image','inspect']:
                ...
                if 'RepoDigests' in fmt:
                    ...
                    if not state['mutated'] and os.getenv('CANDIDATE_DIGEST_BAD') == '1': digests=['other.example/riviera@sha256:'+'9'*64]
                    if state['mutated'] and ref == state['current_id'] and os.getenv('FORWARD_DIGEST_BAD') == '1': digests=['other.example/riviera@sha256:'+'9'*64]
                    print('\\n'.join(digests))
                else:
    ...
    def compose_count(self): return self.logs().count("up -d --force-recreate --pull never riviera-smartpbx")
    def current(self):
...
def test_deploy_accepts_expected_digest_when_repo_digests_are_out_of_order(tmp_path):
    host = FakeDeployHost(tmp_path, candidate_digests=["other.example/riviera@sha256:" + "9" * 64, f"{FakeDeployHost.image}@{FakeDeployHost.digest}"], baseline_digests=["other.example/riviera@sha256:" + "8" * 64, f"{FakeDeployHost.image}@sha256:" + "b" * 64])
    result = host.deploy()
...
def test_deploy_rejects_missing_expected_digest_before_mutation(tmp_path):
    host = FakeDeployHost(tmp_path, candidate_digests=["other.example/riviera@sha256:" + "9" * 64])
    result = host.deploy()
    assert result.returncode != 0 and host.compose_count() == 0
...
EXTRA_NEIGHBOURS = {"SMARTPBX_ISOLATION_NEIGHBOURS": "riviera-voice-agent flico-voice-agent"}


@pytest.mark.parametrize("environment,state", [({"LEGACY_ABSENT": 1}, {}), ({}, {"legacy_health": "unhealthy"})])
def test_deploy_twilio_neighbour_absent_or_unhealthy_blocks_before_mutation(tmp_path, environment, state):
    """The default isolation baseline is Riviera's own Twilio container."""
    host = FakeDeployHost(tmp_path, **state); result = host.deploy(**environment)
    assert result.returncode != 0 and host.compose_count() == 0


@pytest.mark.parametrize("environment,state", [({"FLICO_ABSENT": 1}, {}), ({}, {"flico_health": "unhealthy"})])
def test_deploy_never_depends_on_another_agents_container_by_default(tmp_path, environment, state):
    """Isolation contract (CLAUDE.md): a Riviera deploy never health-gates on
    Kavya's or Flico's container. With the default neighbour set, Flico being
    absent or unhealthy neither blocks the recreate nor is even inspected."""
    host = FakeDeployHost(tmp_path, **state); result = host.deploy(**environment)
    assert result.returncode == 0 and host.compose_count() == 1
    assert "flico-voice-agent" not in host.logs()
    assert "kavya" not in host.logs()


@pytest.mark.parametrize("environment,state", [({"FLICO_ABSENT": 1}, {}), ({}, {"flico_health": "unhealthy"})])
def test_deploy_configured_extra_neighbour_absent_or_unhealthy_blocks_before_mutation(tmp_path, environment, state):
    """SMARTPBX_ISOLATION_NEIGHBOURS widens the baseline: every listed
    container must be present and healthy before anything is mutated."""
    host = FakeDeployHost(tmp_path, **state); result = host.deploy(**EXTRA_NEIGHBOURS, **environment)
    assert result.returncode != 0 and host.compose_count() == 0
...
@pytest.mark.parametrize("environment,protected", [
    ...
    ({"FLICO_ID_CHANGED": 1, **EXTRA_NEIGHBOURS}, "flico_id"),
    ({"FLICO_CHANGED": 1, **EXTRA_NEIGHBOURS}, "flico_health"),
])
...
@pytest.mark.parametrize("environment", [{"FLICO_ID_CHANGED": 1}, {"FLICO_CHANGED": 1}])
def test_deploy_ignores_another_agents_container_changing_during_the_recreate(tmp_path, environment):
    """A Flico recreate or health flap during Riviera's deploy is not an
    isolation violation unless Flico was configured as a neighbour."""
    host = FakeDeployHost(tmp_path); result = host.deploy(**environment)
    assert result.returncode == 0 and host.compose_count() == 1
    assert host.current()["current_id"] == host.candidate
    assert "SMARTPBX_ROLLBACK_ESCALATION_REQUIRED" not in result.stderr
...
def test_deploy_mutates_only_the_pinned_service_and_never_prints_sentinels(tmp_path):
    ...
    assert "up -d --force-recreate --pull never riviera-smartpbx" in host.logs()
    ...
    for forbidden in (" nginx", " prune", " down", " restart", " flico-voice-agent", " riviera-voice-agent"):
        assert all(forbidden not in line for line in mutation_lines)


# Byte-exact capture shape from GHCR, observed 2026-08-08 in workflow run
# https://github.com/taskforce-ai-dev/full-voice-agent/actions/runs/31269079259
# on the throwaway diag/ghcr-capture branch (PR #213, closed unmerged) for the
# kavya image ("ERROR: ghcr.io/taskforce-ai-dev/kavya:<tag>: not found\n").
# The riviera image lives in the same registry namespace and the probe script
# is byte-identical to Kavya's, so the same wording with the riviera reference
# is what an absent riviera tag returns.
OBSERVED_GHCR_ABSENT_CAPTURES = {
    "ghcr.io/taskforce-ai-dev/riviera:37bfaf0":
        "RVJST1I6IGdoY3IuaW8vdGFza2ZvcmNlLWFpLWRldi9yaXZpZXJhOjM3YmZhZjA6IG5vdCBmb3VuZAo=",
    "ghcr.io/taskforce-ai-dev/riviera:probe-0-0-0":
        "RVJST1I6IGdoY3IuaW8vdGFza2ZvcmNlLWFpLWRldi9yaXZpZXJhOnByb2JlLTAtMC0wOiBub3QgZm91bmQK",
}


@pytest.mark.parametrize("target", sorted(OBSERVED_GHCR_ABSENT_CAPTURES))
def test_riviera_image_tag_probe_accepts_the_observed_ghcr_absent_capture(tmp_path, target):
...
def test_riviera_image_tag_probe_allowlist_entries_are_full_capture_matches():
    script = RIVIERA_IMAGE_TAG_PROBE.read_text(encoding="utf-8")
    allowlist_line = next(
...
def test_riviera_image_probe_payload_accepts_an_optional_bootstrap_flag(tmp_path):
    result, summary, log = run_probe_workflow_step(
...
def test_riviera_image_probe_defaults_to_strict_mode(tmp_path):
...
def test_riviera_image_probe_rejects_any_bootstrap_value_but_exactly_true(tmp_path, value):
...
def test_riviera_image_probe_rejects_non_string_bootstrap(tmp_path, value):
    payload = {
        "existing_tag": PROBE_EXISTING_TAG,
        "expected_revision": PROBE_EXPECTED_REVISION,
        "bootstrap": value,
...
def test_riviera_image_probe_bootstrap_accepts_an_absent_existing_tag(tmp_path):
    result, summary, log = run_probe_workflow_step(
    ...
    assert log == f"probe ghcr.io/taskforce-ai-dev/riviera:{PROBE_EXISTING_TAG}\n"
    assert "existing_image=absent" in (tmp_path / "outputs").read_text(encoding="utf-8")


def test_riviera_image_probe_bootstrap_still_verifies_a_tag_that_does_exist(tmp_path):
    result, summary, _log = run_probe_workflow_step(
...
def test_riviera_image_probe_strict_mode_still_rejects_an_absent_existing_tag(tmp_path):
    result, summary, _log = run_probe_workflow_step(
        tmp_path, "Probe known existing tag",
...
def test_riviera_image_probe_bootstrap_still_requires_an_exact_state(tmp_path, code, output):
...
def test_riviera_image_probe_revision_is_skipped_with_a_reason_when_there_is_no_image(tmp_path):
    result, summary, log = run_probe_workflow_step(
...
def test_riviera_image_probe_revision_still_verifies_when_the_image_exists(tmp_path):
...
def test_riviera_image_probe_summary_discloses_the_mode(tmp_path, bootstrap, expected_mode):
...
def test_stt_endpointing_knobs_are_deliverable_through_the_documented_path():
    ...
    # RIVIERA_EN_ELEVENLABS_VOICE_ID class of bug. The riviera (Twilio) service uses
    # env_file: .env, so .env.example is its passthrough.
    compose = yaml.safe_load(read_text("docker-compose.yml"))
    smartpbx_env = compose["services"]["riviera-smartpbx"]["environment"]
    riviera = compose["services"]["riviera"]
    assert ".env" in riviera["env_file"], "the Twilio service forwards .env vars via env_file"
...
def test_pilot_transcript_logging_is_explicit_off_by_default_and_reversible():
    ...
    assert "role=riviera" in runbook
...
def test_dtmf_knobs_are_deliverable_through_the_documented_path():
    ...
    # but not listed there never reaches the container. The riviera (Twilio) service
...
def test_generic_deploy_and_pr_impact_exclude_riviera_from_image_routing():
    """Riviera images have a separately reviewed probe -> publisher -> runbook path."""
    ...
    # The generic matrix must never carry a Riviera image row into deploy.yml.
    assert 'items="${items}{\\"agent\\":\\"${id}\\",\\"mode\\":\\"${mode}\\"},"' in deploy_on_push
    riviera_guard = deploy_on_push.split('[ "$id" = "riviera" ]', 1)[1]
    assert riviera_guard.find('continue') < riviera_guard.find('items="${items}')


def test_riviera_exclusion_keeps_other_registry_agents_on_generic_image_routing():
    workflows = PROJECT_ROOT.parent / ".github" / "workflows"
    deploy_on_push = (workflows / "deploy-on-push.yml").read_text(encoding="utf-8")
    pr_impact = (workflows / "pr-deploy-impact.yml").read_text(encoding="utf-8")
...
def test_riviera_generic_rejection_and_reviewed_image_gates_remain_intact():
    workflows = PROJECT_ROOT.parent / ".github" / "workflows"
    generic_deploy = (workflows / "deploy.yml").read_text(encoding="utf-8")
    probe = (workflows / "probe-riviera-image.yml").read_text(encoding="utf-8")
    publisher = (workflows / "build-riviera-image.yml").read_text(encoding="utf-8")

    assert '"$AGENT" == "riviera"' in generic_deploy
    assert "is excluded from generic deploy.yml for every mode" in generic_deploy

    assert "repository_dispatch:" in probe
    assert "riviera_image_read_only_probe" in probe
...
def test_sinhala_smartpbx_settings_are_explicit_and_secret_safe():
    compose = yaml.safe_load(read_text("docker-compose.yml"))
    smartpbx = compose["services"]["riviera-smartpbx"]["environment"]
    legacy = compose["services"]["riviera"]
...
def test_sinhala_azure_segmentation_canary_is_allowlisted_and_reversible():
    compose = yaml.safe_load(read_text("docker-compose.yml"))
    smartpbx = compose["services"]["riviera-smartpbx"]["environment"]
    legacy = compose["services"]["riviera"]["environment"]
...
def test_sinhala_smartpbx_runbook_documents_the_llm_tts_and_transaction_contract():
    ...
    for required in (
        ...
        "no-output",
        "stripped",
        "before exposing the language menu",
        "offline-only",
        "Gemini Transcribe",
        "Chirp 2",
        "StreamingRecognize",
        "update_smartpbx_sinhala_provider.sh apply /opt/riviera/.env.smartpbx claude",
        "update_smartpbx_sinhala_provider.sh restore /opt/riviera/.env.smartpbx",
        "update_smartpbx_sinhala_provider.sh cleanup /opt/riviera/.env.smartpbx",
        "config validation failure",
        "post-recreate failure",
        "same reviewed identity",
        "health/status checks",
        "three-language canary call checklist",
        "press `3`",
    ):
...
def test_smartpbx_sinhala_llm_rendered_compose_and_protected_template_contract(tmp_path):
    ...
    assert set(defaults) == {"riviera", "riviera-smartpbx"}
    assert {
        name: defaults["riviera-smartpbx"]["environment"][name]
        for name in SINHALA_LLM_DEFAULTS
    } == SINHALA_LLM_DEFAULTS
    assert not set(SINHALA_LLM_DEFAULTS).intersection(defaults["riviera"]["environment"])
    ...
    assert {
        name: rendered_overrides["riviera-smartpbx"]["environment"][name]
        for name in overrides
    } == overrides
    assert not set(overrides).intersection(rendered_overrides["riviera"]["environment"])
...
def test_sinhala_provider_updater_is_a_closed_redacted_root_transaction():
    ...
    for required in (
        "PROTECTED_FILE=/opt/riviera/.env.smartpbx",
        "TRANSACTION_DIR=/opt/riviera/.smartpbx-sinhala-rollback",
        "[[ $EUID -eq 0 ]]",
    ...
    for required in (
        ...
        "sudo test -x /opt/riviera/scripts/update_smartpbx_sinhala_provider.sh",
        "sudo chown root:root /opt/riviera/scripts/update_smartpbx_sinhala_provider.sh",
        "sudo chmod 0755 /opt/riviera/scripts/update_smartpbx_sinhala_provider.sh",
        "sudo install -d -o root -g root -m 0700 /opt/riviera/.smartpbx-sinhala-rollback",
    ):
        assert required in runbook


def test_sinhala_provider_updater_transaction_uses_only_its_private_backup(tmp_path):
    """Exercise a path-rewritten copy: never /opt/riviera or a real credential.
...
""",
    ...
    updater = updater.replace("/opt/riviera/.env.smartpbx", str(protected))
    updater = updater.replace("/opt/riviera/.smartpbx-sinhala-rollback", str(transaction))
...
def test_documented_sinhala_rollback_transaction_orders_failures_and_cleanup(tmp_path):
    ...
    def scenario(name: str, **switches: str) -> tuple[subprocess.CompletedProcess[str], list[str], Path, Path]:
        ...
        fake(
            ...
            '  curl --fail http://127.0.0.1:8061/health | jq -e . >/dev/null\n'
            '  curl --fail http://127.0.0.1:8061/smartpbx/status | jq -e . >/dev/null\n'
            '  rm -f "$TX"/*\n'
            'else exit 1\nfi\n',
        )
        fake(
        ...
        # The runbook invokes fixed /opt/riviera script paths. Point those paths
        ...
        driver.write_text(
            ...
            + runbook_script.replace("/opt/riviera", str(app)),
            encoding="utf-8",
        )
        driver.chmod(0o755)
        env = {
...
def test_riviera_smartpbx_compose_runtime_hardening_is_pinned():
    ...
    assert service["ports"] == ["127.0.0.1:8061:8000"]
    legacy = compose["services"]["riviera"]
