...
GREEN production-only: implement HangupEvent(legs, optional reason), DtmfEvent(legs,digit,duration), A-D, value-free UnsupportedEvent, Start three-field and Hangup/DTMF two-leg validation. Add temporary value-free compatibility alias UnknownEvent = UnsupportedEvent (or equivalent type alias): parser returns UnsupportedEvent and never retains a name, while Kavya/smartpbx_gateway.py imports/tests remain coherent until Task 3. Command: cd Kavya && /home/dev/incoming/taskforce-ai/.venv/bin/python -B -m py_compile smartpbx_protocol.py && PYTHONDONTWRITEBYTECODE=1 PYTEST_ADDOPTS="-p no:cacheprovider" /home/dev/incoming/taskforce-ai/.venv/bin/python -m pytest tests -q && docker build -f Dockerfile -t kavya-smartpbx-task1-import . && docker run --rm --entrypoint python kavya-smartpbx-task1-import -c "import smartpbx_protocol, smartpbx_gateway, server". Expected pass. Commit only protocol.py.
...
RED tests-only: transition-safe factories are (context, transport, sink=None), capture sink and assert current gateway default None makes the required non-None callable SmartPBXDiagnosticSink diagnostic-contract assertion fail. Direct-session tests retain constructor calls or use getattr/local capture. Cover seven-field record, correlation format/uniqueness/no-sentinel, table, unsupported pre/active, DTMF observed/mismatch, counter removal, first terminal later-media unread, cancellation. Inject finish/transport/release/close faults: every applicable operation exactly once, later cleanup continues, released_total exactly 1 when release works, single close. Command: cd Kavya && PYTHONDONTWRITEBYTECODE=1 PYTEST_ADDOPTS="-p no:cacheprovider" /home/dev/incoming/taskforce-ai/.venv/bin/python -m pytest tests/test_smartpbx_gateway.py tests/test_smartpbx_gateway_transfer_pending.py tests/test_smartpbx_real_handover_lifecycle.py tests/test_smartpbx_server.py -q. Expected meaningful assertion failure while current gateway supplies None; after GREEN the typed non-None sink emits the exact diagnostic contract. Commit tests only.

GREEN production-only: change SessionFactory at Kavya/smartpbx_gateway.py:143 and server factory to three args; gateway supplies typed non-None sink. KavyaSmartPBXSession constructor accepts optional/no-op sink for unrelated direct handover compatibility and installs before welcome. Update every inventory site; replace gateway UnknownEvent imports/branches/count with UnsupportedEvent rejection, remove the temporary UnknownEvent alias, and add final test/assertion that UnknownEvent is not exported. Implement correlation/table/counter removal/ordered cleanup. Command: cd Kavya && PYTHONDONTWRITEBYTECODE=1 PYTEST_ADDOPTS="-p no:cacheprovider" /home/dev/incoming/taskforce-ai/.venv/bin/python -m pytest tests -q. Expected pass. Commit production files only.
