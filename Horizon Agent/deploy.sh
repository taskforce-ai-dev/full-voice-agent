...
DOMAIN="hattonhills.taskforceai.tech"
EMAIL="info@taskforceai.tech"
# ======================================================

SERVER_USER="root"
REMOTE_DIR="/opt/hatton-hills"
...
cmd_setup() {
  check_config
  info "Setting up $SERVER_IP for Hatton Hills deployment..."
...
UFW_SETUP
  ...
  run_remote "bash -s" <<NGINX_TEMP
    cat > /etc/nginx/sites-available/hatton-hills <<'CONF'
server {
    listen 80;
    server_name ${DOMAIN};
    location / {
        return 200 'Hatton Hills setup in progress';
        add_header Content-Type text/plain;
    }
}
CONF
    ln -sf /etc/nginx/sites-available/hatton-hills /etc/nginx/sites-enabled/
...
NGINX_TEMP
  ...
  run_remote "sed 's/YOUR_DOMAIN/${DOMAIN}/g' ${REMOTE_DIR}/nginx.conf > /etc/nginx/sites-available/hatton-hills && nginx -t && systemctl reload nginx"
...
cmd_logs() {
  check_config
  run_remote "cd ${REMOTE_DIR} && docker compose logs -f hatton-hills"
}
