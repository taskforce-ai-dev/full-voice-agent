...
The agent persona is **Kavya** — a warm, multilingual reservations agent. The live IVR/DTMF menu (as of v0.16) offers **press 1 = English, press 2 = Arabic, press 3 = Sinhala** — `DIGIT_TO_LANG = {"1": "en", "2": "ar", "3": "si"}`. Sinhala TTS now uses **OpenAI `gpt-4o-mini-tts`** (voice `nova`) as of v0.16 (was Azure `si-LK-SameeraNeural`). Tamil is fully implemented in code (Media Streams path below) but is still **NOT surfaced in the menu** — add `"4": "ta"` (and a `<Say>` prompt) to expose it.
