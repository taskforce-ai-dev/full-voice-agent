...
`capture_spoken_number` tool override) as ONE utterance. For Direct SmartPBX
Sinhala only, that utterance must be normalised to plain digits FIRST, so a
caller who says "හැට පහ හතර තුන" (sixty five, four, three) is captured just
like an English caller who dictates "six five four three".
"""
