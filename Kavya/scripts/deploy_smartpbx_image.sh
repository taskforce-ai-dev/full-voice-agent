...
check_loopback_preflight(){ local status_json; curl --silent --show-error --fail --connect-timeout 2 --max-time 5 http://127.0.0.1:8006/health >/dev/null 2>&1 || return 1; status_json=1000 27 100 1000curl --silent --show-error --fail --connect-timeout 2 --max-time 5 http://127.0.0.1:8006/smartpbx/status 2>/dev/null) || return 1; jq -e ".active_sessions == 0 and .transfer_enabled == false" >/dev/null <<<"" || return 1; }
